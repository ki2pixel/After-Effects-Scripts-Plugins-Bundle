...
(function () {
  ...
  function light_keys(data, blender_lights) {
    for (var ob in data[blender_lights]) {
      if (data[blender_lights].hasOwnProperty(ob)) {
        var newLight = myComp.layers.addLight(ob, [0, 0]);
        newLight.autoOrient = AutoOrientType.NO_AUTO_ORIENT;
        newLight
          .property("ADBE Light Options Group")
          .property("ADBE Casts Shadows")
          .setValue(true);
        newLight.name = ob;
        newLight.outPoint = data.duration / fps;
        if (data[blender_lights][ob].type == "SPOT") {
          newLight.lightType = LightType.SPOT;
        } else {
          newLight.lightType = LightType.POINT;
        }
        for (var prop in data[blender_lights][ob]) {
          n = data[blender_lights][ob][prop].length;
          if (isTrial && n > fps) {
            n = fps;
            isAnim = true;
          }
          if (prop == "colour") {
            for (var i = 0; i < n; i += 1) {
              newLight
                .property("ADBE Light Options Group")
                .property("ADBE Light Color")
                .setValueAtTime(i / fps, data[blender_lights][ob][prop][i]);
            }
          }
          if (prop == "energy") {
            for (var i = 0; i < n; i += 1) {
              newLight
                .property("ADBE Light Options Group")
                .property("ADBE Light Intensity")
                .setValueAtTime(i / fps, data[blender_lights][ob][prop][i]);
            }
          }
          if (newLight.lightType == LightType.SPOT && prop == "cone_angle") {
            for (var i = 0; i < n; i += 1) {
              newLight
                .property("ADBE Light Options Group")
                .property("ADBE Light Cone Angle")
                .setValueAtTime(i / fps, data[blender_lights][ob][prop][i]);
            }
          }
          if (newLight.lightType == LightType.SPOT && prop == "cone_feather") {
            for (var i = 0; i < n; i += 1) {
              newLight
                .property("ADBE Light Options Group")
                .property("ADBE Light Cone Feather 2")
                .setValueAtTime(i / fps, data[blender_lights][ob][prop][i]);
            }
          }
          if (prop == "position") {
  ...
  function cam_keys(data, blender_cams) {
    var i = 0;
    for (var ob in data[blender_cams]) {
      if (data[blender_cams].hasOwnProperty(ob)) {
        var newCam = myComp.layers.addCamera(ob, [0, 0]);
        newCam.autoOrient = AutoOrientType.NO_AUTO_ORIENT;
        newCam.name = ob;
        if (data.cam_markers && data.cam_markers.length) {
          for (var i = 0; i < data.cam_markers.length; i += 1) {
            if (data.cam_markers[i][1] == newCam.name) {
              newCam.inPoint = (data.cam_markers[i][0] - 1) / fps;
              newCam.outPoint = data.cam_markers[i][2] / fps;
              var startFrame = data.cam_markers[i][0] - 1;
              var endFrame = data.cam_markers[i][2];
            } else {
              continue;
            }
          }
        } else {
          newCam.outPoint = data.duration / fps;
          var startFrame = 0;
          var endFrame = data.duration;
        }
        for (var prop in data[blender_cams][ob]) {
          if (data[blender_cams][ob].hasOwnProperty(prop)) {
            for (var i = startFrame; i < endFrame; i++) {
              if (data[blender_cams][ob][prop].length == 1) {
                var propPosition = 0;
              } else {
                var propPosition = i;
              }
              if (prop == "zoom") {
                newCam
                  ...
                  .setValueAtTime(
                    i / fps,
                    data[blender_cams][ob][prop][propPosition],
                  );
  ...
  function null_keys(data, blender_objects) {
    var i = 0;
    for (var ob in data[blender_objects]) {
      if (data[blender_objects].hasOwnProperty(ob)) {
        var newNull = myComp.layers.addNull();
        newNull.source.name = ob;
        newNull.threeDLayer = true;
        newNull.outPoint = data.duration / fps;
        for (var prop in data[blender_objects][ob]) {
          if (data[blender_objects][ob].hasOwnProperty(prop)) {
            var n = data[blender_objects][ob][prop].length;
            if (isTrial && n > fps) {
              n = fps;
              isAnim = true;
            }
            if (prop == "position") {
              for (var i = 0; i < n; i += 1) {
                newNull
                  .property("ADBE Transform Group")
                  .property("ADBE Position")
                  .setValueAtTime(i / fps, data[blender_objects][ob][prop][i]);
              }
            }
            if (prop == "rotation") {
              for (var i = 0; i < n; i += 1) {
  ...
  function faces_keys(data, blender_faces) {
    myComp.hideShyLayers = true;
    var i = 0;
    var x = data.width / 2;
    var y = data.height / 2;
    var center = [x, y, 0];
    for (var ob in data[blender_faces]) {
      if (
        data[blender_faces][ob].face_loc_global.length > 300 ||
        data[blender_faces][ob].face_loc_global[0].length > 2
      ) {
        alert(
          "Warning: Importing " +
            data[blender_faces][ob].face_loc_global[0].length +
            " faces and processing " +
            data[blender_faces][ob].face_loc_global.length +
            " frames may take some time. \nPress \'esc\' during the import process to abort.",
        );
      }
      var controllerNull = myComp.layers.addNull();
      controllerNull.source.name = ob + "_CONTROL";
      controllerNull.threeDLayer = true;
      controllerNull.outPoint = data.duration / fps;
      var newNull = myComp.layers.addNull();
      newNull.source.name = ob;
      newNull.threeDLayer = true;
      newNull.shy = true;
      newNull.outPoint = data.duration / fps;
      for (var prop in data[blender_faces][ob]) {
        n = data[blender_faces][ob][prop].length;
        if (prop == "position") {
          for (var i = 0; i < n; i += 1) {
            newNull
              .property("ADBE Transform Group")
              .property("ADBE Position")
              .setValueAtTime(i / fps, data[blender_faces][ob][prop][i]);
          }
        }
        if (prop == "rotation") {
          for (var i = 0; i < n; i += 1) {
      ...
      controllerNull
        ...
        .property("ADBE Position")
        .setValue(conPos);
      var faceLen = data[blender_faces][ob].face_loc_global[0].length;
      for (var face = 0; face < faceLen; face += 1) {
        var faceNull = myComp.layers.addNull();
        faceNull.source.name = ob + "_face_" + face;
        faceNull.threeDLayer = true;
        faceNull.shy = true;
        faceNull.enabled = false;
        faceNull.outPoint = data.duration / fps;
        n = data[blender_faces][ob].face_loc_global.length;
        if (isTrial && n > fps) {
          n = fps;
          isAnim = true;
        }
        for (var i = 0; i < n; i += 1) {
          faceNull
            ...
            .setValueAtTime(
              i / fps,
              data[blender_faces][ob].face_loc_global[i][face],
            );
        }
        n = data[blender_faces][ob].face_rot_global.length;
        if (isTrial && n > fps) {
          n = fps;
          isAnim = true;
        }
        for (var i = 0; i < n; i += 1) {
          faceNull
            ...
            .setValueAtTime(
              i / fps,
              data[blender_faces][ob].face_rot_global[i][face],
            );
        }
        var vert_len =
          data[blender_faces][ob].face_verts_global[0][face].length;
        var vertArr = [];
        var vertPosArr = [];
        for (var v = 0; v < vert_len; v += 1) {
          var vertNull = myComp.layers.addNull();
          vertNull.source.name = ob + " face_" + face + " vert_" + v;
          vertNull.threeDLayer = true;
          vertNull.shy = true;
          vertNull.enabled = false;
          vertNull.outPoint = data.duration / fps;
          vertArr.push(vertNull);
          n = data[blender_faces][ob].face_verts_global.length;
          if (isTrial && n > fps) {
            n = fps;
            isAnim = true;
          }
          for (var i = 0; i < n; i += 1) {
            vertNull
              ...
              .setValueAtTime(
                i / fps,
                data[blender_faces][ob].face_verts_global[i][face][v],
              );
          }
        }
        var myShapeLayer = myComp.layers.addShape();
        myShapeLayer.name = ob + "_faceShape_" + face;
        myShapeLayer.outPoint = data.duration / fps;
        ...
        var myPathGroup = myShapeContents.addProperty(
          "ADBE Vector Shape - Group",
        );
        var myPath = myPathGroup.property("ADBE Vector Shape");
        var vectors = new Shape();
        vectors.closed = true;
        if (
          data[blender_faces][ob].shape_keys == false &&
          data[blender_faces][ob].scale.length == 1
        ) {
          n = 1;
        } else {
          n = data[blender_faces][ob].face_loc_global.length;
        ...
        for (var i = 0; i < n; i += 1) {
          myComp.time = i / fps;
          for (var k = 0; k < vertArr.length; k += 1) {
            vertArr[k].parent = faceNull;
          }
          facePos = faceNull
            .property("ADBE Transform Group")
            .property("ADBE Position")
            .valueAtTime(i / fps, false);
          faceRot = faceNull
            .property("ADBE Transform Group")
            .property("ADBE Orientation")
            .valueAtTime(i / fps, false);
          faceNull
            .property("ADBE Transform Group")
            .property("ADBE Orientation")
            .setValueAtTime(i / fps, [0, 0, 0]);
          faceNull
            .property("ADBE Transform Group")
            .property("ADBE Position")
            .setValueAtTime(i / fps, center);
          for (var k = 0; k < vertArr.length; k += 1) {
            var vertPos = vertArr[k]
              .property("ADBE Transform Group")
              .property("ADBE Position")
              .valueAtTime(i / fps, false);
            vertPosArr.push([vertPos[0], vertPos[1]]);
          }
          faceNull
            .property("ADBE Transform Group")
            .property("ADBE Position")
            .setValueAtTime(i / fps, facePos);
          faceNull
            .property("ADBE Transform Group")
            .property("ADBE Orientation")
            .setValueAtTime(i / fps, faceRot);
          if (data[blender_faces][ob].face_verts_global.length == 1) {
            for (var k = 0; k < vertArr.length; k += 1) {
              vertArr[k].parent = newNull;
              vertArr[k]
                .property("ADBE Transform Group")
                .property("ADBE Scale")
                .setValue([20, 20, 20]);
            ...
            for (var k = 0; k < vertArr.length; k += 1) {
              vertArr[k].parent = controllerNull;
              vertArr[k]
                .property("ADBE Transform Group")
                .property("ADBE Scale")
                .setValue([20, 20, 20]);
          ...
          vectors.vertices = vertPosArr;
          myPath.setValueAtTime(i / fps, vectors);
          vertPosArr = [];
        }
        myShapeLayer.threeDLayer = true;
        var myFillColor = [0.45, 0.75, 0.9];
        ...
        var precompLayerIndex = myShapeLayer.index;
        myComp.layers.precompose([myShapeLayer.index], myShapeLayer.name, true);
        var precompLayer = myComp.layer(precompLayerIndex);
        precompLayer.threeDLayer = true;
        precompLayer.collapseTransformation = true;
        myComp.time = 0;
        precompLayer.parent = faceNull;
        precompLayer
          .property("ADBE Transform Group")
          .property("ADBE Position")
          .setValue([0, 0, 0]);
        precompLayer
          ...
          .property("ADBE Orientation")
          .setValue([0, 0, 0]);
        faceNull.parent = controllerNull;
      }
      newNull.parent = controllerNull;
    }
  }
  function vert_keys(data, blender_verts) {
    var i = 0;
    for (var ob in data[blender_verts]) {
      var newNull = myComp.layers.addNull();
      newNull.source.name = ob;
      newNull.threeDLayer = true;
      newNull.outPoint = data.duration / fps;
      for (var prop in data[blender_verts][ob]) {
        n = data[blender_verts][ob][prop].length;
        if (isTrial && n > fps) {
          n = fps;
          isAnim = true;
        }
        if (prop == "position") {
          for (var i = 0; i < n; i += 1) {
            newNull
              .property("ADBE Transform Group")
              .property("ADBE Position")
              .setValueAtTime(i / fps, data[blender_verts][ob][prop][i]);
          }
        }
        if (prop == "rotation") {
          for (var i = 0; i < n; i += 1) {
      ...
      var vLength = data[blender_verts][ob].vert_locations[0].length;
      for (var v = 0; v < vLength; v += 1) {
        var vertNull = myComp.layers.addNull();
        vertNull.source.name = ob + " vert_" + v;
        vertNull.threeDLayer = true;
        vertNull.outPoint = data.duration / fps;
        n = data[blender_verts][ob].vert_locations.length;
        if (isTrial && n > fps) {
          n = fps;
          isAnim = true;
        }
        for (var i = 0; i < n; i += 1) {
          vertNull
            ...
            .setValueAtTime(
              i / fps,
              data[blender_verts][ob].vert_locations[i][v],
            );
        }
        if (n == 1) {
          myComp.time = 0;
          vertNull.parent = newNull;
          vertNull
            .property("ADBE Transform Group")
            .property("ADBE Scale")
            .setValue([20, 20, 20]);
  ...
  function check_comp(data) {
    ...
    if (app.project.activeItem == null) {
      return create_comp(data);
    } else {
      myComp = app.project.activeItem;
    }
    if (
      data.width != myComp.width ||
      data.height != myComp.height ||
      data.aspect != myComp.pixelAspect ||
      data.fps != myComp.frameRate
    ) {
      return create_comp(data);
    }
    return myComp;
  ...
  function create_comp(data) {
    var compName = "BlenderAe_" + data.fileDir[1];
    myComp = app.project.items.addComp(
      compName,
      data.width,
      data.height,
      data.aspect,
  ...
  function a(vars) {
    ...
    var maxUIButtons = 3;
    var licV = 2;
    var mx = __BLOB__BLOB_000100__;
    var wx = __BLOB__BLOB_000101__;
  ...
  var af_settings = {
    ...
    helpButtons: [],
    helpText:
      "BlenderAe is installed and run as an Addon in Blender. \nFor Addon details in Blender go to \'Edit > Preferences > Addons > Import-Export: BlenderAe\'.",
    offerBeta: false,
    offerTrial: true,
  ...
  if (smBAe1.c()) {
    var isTrial = smBAe1.t();
    var isAnim = false;
    var jsonPath = File(
      Folder.myDocuments.toString() + "/BlenderAe/BlenderAe_data.json",
    );
    var data = read_json(jsonPath);
    app.beginUndoGroup("Blender_Ae");
    var myComp = check_comp(data);
    var fps = myComp.frameRate;
    if (data) {
      if (smBAe1.s() === false) {
        isTrial = true;
        alert(
          "Thank you for trialling BlenderAe! \nPlease note that a maximum of 1 second of animation data is imported in trial mode.",
        );
      }
      null_keys(data, "nulls");
      light_keys(data, "lights");
      cam_keys(data, "cameras");
      faces_keys(data, "solids");
      faces_keys(data, "faces");
      vert_keys(data, "verts_only");
    } else {
      alert("BlenderAe data is missing");
    }
    app.endUndoGroup();
  }
})();
