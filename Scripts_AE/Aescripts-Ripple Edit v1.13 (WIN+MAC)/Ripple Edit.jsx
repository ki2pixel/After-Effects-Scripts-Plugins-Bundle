...
(function (thisObj) {
  function alertError(err, fileName) {
    alert(
      [
        err.name,
        err.message,
        [err.fileName, fileName, err.line].join(":"),
        "---",
        $.stack,
      ].join("\n"),
    );
  ...
  function buildUI() {
    clearOutput();
    writeLn("Ripple Edit");
    writeLn("By Plugin Everything");
    writeLn("V1.1.2");
    cntr = thisObj instanceof Panel ? thisObj : new Window("palette");
    cntr.orientation = "column";
    cntr.alignChildren = ["left", "top"];
    cntr.spacing = 1;
    keyState = ScriptUI.environment.keyboardState;
    images();
    if (!app.settings.haveSetting("Ripple Edit", "Orientation")) {
      app.settings.saveSetting("Ripple Edit", "Orientation", "column");
    }
    cntr.orientation = app.settings.getSetting("Ripple Edit", "Orientation");
    btn_inPoint = iconButton(cntr, {
      down: img_inPoint[2],
      rollover: img_inPoint[1],
      standard: img_inPoint[0],
    });
    btn_inPoint.helpTip =
      "Shifts the In Point of selected layers right by 1 frame adjusting layers to fit\nAlt - Shifts left\nShift - Shifts by 10 frames\nCtrl/Cmd - Shifts, but doesn\'t adjust other layers";
    btn_closegap = iconButton(cntr, {
      down: img_closegap[2],
      rollover: img_closegap[1],
      standard: img_closegap[0],
    });
    btn_closegap.helpTip =
      "Closes the gap between layers sequentially\nAlt - Closes the gap between one layer and all layers to the right as a group";
    btn_delete = iconButton(cntr, {
      disabled: img_delete[3],
      down: img_delete[2],
      rollover: img_delete[1],
      standard: img_delete[0],
    });
    btn_delete.helpTip =
      "Deletes selected layers, shifting layers to keep consistent positioning";
    btn_activepoint = iconButton(cntr, {
      down: img_activepoint[2],
      rollover: img_activepoint[1],
      standard: img_activepoint[0],
    });
    btn_activepoint.helpTip =
      "Sets the In Point of selected layers to the current time, shifting layers to match\nAlt - Sets the Out Point instead of the In Point";
    btn_toedges = iconButton(cntr, {
      down: img_toedges[2],
      rollover: img_toedges[1],
      standard: img_toedges[0],
    });
    btn_toedges.helpTip =
      "Sets the In and Out Points of selected layers to the bounds of the comp\nAlt - Sets the bounds of the comp to the In and Out Points of selected layers";
    btn_outPoint = iconButton(cntr, {
      down: img_outPoint[2],
      rollover: img_outPoint[1],
      standard: img_outPoint[0],
    });
    btn_outPoint.helpTip =
      "Shifts the Out Point of selected layers left by 1 frame adjusting layers to fit\nAlt - Shifts right\nShift - Shifts by 10 frames\nCtrl/Cmd - Shifts, but doesn\'t adjust other layers";
    btn_help = iconButton(cntr, {
      down: img_help[2],
      rollover: img_help[1],
      standard: img_help[0],
    });
    btn_help.helpTip = "Opens the help and licensing menu";
    if (peS.t()) {
      btn_delete.enabled = false;
    }
    cntr.addEventListener("click", function (event) {
      if (event.detail != 2) {
        return;
      }
      keyState = ScriptUI.environment.keyboardState;
      if (keyState.altKey != false) {
        clearOutput();
        if (cntr.orientation == "column") {
          cntr.orientation = "row";
          app.settings.saveSetting("Ripple Edit", "Orientation", "row");
          writeLn("Ripple Edit orintation set to row");
        } else {
          cntr.orientation = "column";
          app.settings.saveSetting("Ripple Edit", "Orientation", "column");
          writeLn("Ripple Edit orintation set to column");
        }
        cntr.layout.layout(true);
      }
    });
    btn_help.onClick = function () {
      clearOutput();
      writeLn("Opening Ripple Edit help menu");
      peS.helpUI();
      if (peS.t()) {
        btn_delete.enabled = false;
      }
      writeLn("Help menu closed, settings saved");
    };
    btn_inPoint.onClick = function () {
      keyState = ScriptUI.environment.keyboardState;
      var duration = 1;
      if (keyState.shiftKey && !peS.t()) {
        duration = 10;
      }
      if (keyState.altKey && !peS.t()) {
        duration = duration * -1;
      }
      if (keyState.ctrlKey || keyState.metaKey) {
        inPoint(duration, false);
      } else {
        inPoint(duration, true);
      }
      if (peS.t()) {
        alert(
          "You are currently running Ripple Edit in trial mode\\nDelete, Alternative and extra functions are disabled",
        );
      }
    };
    btn_closegap.onClick = function () {
      keyState = ScriptUI.environment.keyboardState;
      if (keyState.altKey && !peS.t()) {
        closeGap(true);
      } else {
    ...
    btn_activepoint.onClick = function () {
      keyState = ScriptUI.environment.keyboardState;
      if (keyState.altKey && !peS.t()) {
        activePoint(true);
      } else {
    ...
    btn_outPoint.onClick = function () {
      ...
      if (keyState.ctrlKey || keyState.metaKey) {
        outPoint(duration, false);
      } else {
        outPoint(duration, true);
      }
      if (peS.t()) {
        alert(
          "You are currently running Ripple Edit in trial mode\\nDelete, Alternative and extra functions are disabled",
    ...
    cntr.layout.layout(true);
    cntr.layout.resize();
    cntr.onResizing = cntr.onResize = function () {
      this.layout.resize();
    };
    return cntr;
  }
  function iconButton(par, imgs) {
    try {
      var st = imgs.standard || undefined;
      var rl = imgs.rollover || undefined;
      var dn = imgs.down || undefined;
      var ds = imgs.disabled || undefined;
      var btn = par.add("customView{}");
      btn.size = [22, 22];
      var stI = ScriptUI.newImage(st);
      btn.image = stI;
      if (rl != undefined) {
        rlI = ScriptUI.newImage(rl);
      ...
      btn.onDraw = function () {
        var img = this.image;
        if (!this.enabled) {
          this.image = this.disabledImg || img;
        }
      ...
      try {
        throw Exception.log(err, MAIN_JSX);
      } catch (_) {
        alertError(err, MAIN_JSX);
      }
    }
  }
  function images() {
    function getUserDataFolder() {
      ...
      var aescriptsFolder = Folder(
        userDataFolder.toString() + "/Aescripts/PluginEverything/Ripple_Edit",
      );
    ...
    function createParents(path) {
      if (!path.parent.exists) {
        path.parent.create();
        createParents(path.parent);
      }
    }
    ...
    img_outPoint = [];
    img_help = [];
    var iP_st = __BLOB__BLOB_000621__;
    var iP_rl = __BLOB__BLOB_000622__;
    var iP_dn = __BLOB__BLOB_000623__;
    var cG_st = __BLOB__BLOB_000624__;
    var cG_rl = __BLOB__BLOB_000625__;
    var cG_dn = __BLOB__BLOB_000626__;
    var de_st = __BLOB__BLOB_000627__;
    var de_rl = __BLOB__BLOB_000628__;
    var de_dn = __BLOB__BLOB_000629__;
    var de_ds = __BLOB__BLOB_000630__;
    var aP_st = __BLOB__BLOB_000631__;
    var aP_rl = __BLOB__BLOB_000632__;
    var aP_dn = __BLOB__BLOB_000633__;
    var tE_st = __BLOB__BLOB_000634__;
    var tE_rl = __BLOB__BLOB_000635__;
    var tE_dn = __BLOB__BLOB_000636__;
    var oP_st = __BLOB__BLOB_000637__;
    var oP_rl = __BLOB__BLOB_000638__;
    var oP_dn = __BLOB__BLOB_000639__;
    var hp_st = __BLOB__BLOB_000640__;
    var hp_rl = __BLOB__BLOB_000641__;
    var hp_dn = __BLOB__BLOB_000642__;
    var iP_st_lr = __BLOB__BLOB_000643__;
    var iP_rl_lr = __BLOB__BLOB_000644__;
    var iP_dn_lr = __BLOB__BLOB_000645__;
    var cG_st_lr = __BLOB__BLOB_000646__;
    var cG_rl_lr = __BLOB__BLOB_000647__;
    var cG_dn_lr = __BLOB__BLOB_000648__;
    var de_st_lr = __BLOB__BLOB_000649__;
    var de_rl_lr = __BLOB__BLOB_000650__;
    var de_dn_lr = __BLOB__BLOB_000651__;
    var de_ds_lr = __BLOB__BLOB_000652__;
    var aP_st_lr = __BLOB__BLOB_000653__;
    var aP_rl_lr = __BLOB__BLOB_000654__;
    var aP_dn_lr = __BLOB__BLOB_000655__;
    var tE_st_lr = __BLOB__BLOB_000656__;
    var tE_rl_lr = __BLOB__BLOB_000657__;
    var tE_dn_lr = __BLOB__BLOB_000658__;
    var oP_st_lr = __BLOB__BLOB_000659__;
    var oP_rl_lr = __BLOB__BLOB_000660__;
    var oP_dn_lr = __BLOB__BLOB_000661__;
    var hp_st_lr = __BLOB__BLOB_000662__;
    var hp_rl_lr = __BLOB__BLOB_000663__;
    var hp_dn_lr = __BLOB__BLOB_000664__;
    var userDataFolder = getUserDataFolder();
    var aeVersion = "";
    var aeVersionArry = [""];
    var aeNumber = 0;
    aeVersion = app.version;
    aeVersion = aeVersion.substr(0, aeVersion.indexOf("x"));
    aeVersionArry = aeVersion.split(".");
    aeVersion = aeVersionArry[0] + "." + aeVersionArry[1];
    aeNumber = Number(aeVersion);
    if (aeNumber < 13) {
      img_inPoint.push(
        createResourceFile(
          "inPoint/inPoint_Standard_LR.png",
          iP_st,
          userDataFolder,
        ),
      );
      ...
      img_closegap.push(
        createResourceFile(
          "closeGap/closeGap_Standard_LR.png",
          cG_st,
          userDataFolder,
        ),
      );
      img_closegap.push(
        createResourceFile(
          "closeGap/closeGap_Hover_LR.png",
          cG_rl,
          userDataFolder,
        ),
      );
      ...
      img_delete.push(
        createResourceFile(
          "delete/delete_Standard_LR.png",
          de_st,
          userDataFolder,
        ),
      );
      img_delete.push(
        createResourceFile("delete/delete_Hover_LR.png", de_rl, userDataFolder),
      );
      img_delete.push(
        createResourceFile("delete/delete_Down_LR.png", de_dn, userDataFolder),
      );
      img_activepoint.push(
        createResourceFile(
          "activePoint/activePoint_Standard_LR.png",
          aP_st,
          userDataFolder,
        ),
      );
      img_activepoint.push(
        createResourceFile(
          "activePoint/activePoint_Hover_LR.png",
          aP_rl,
          userDataFolder,
        ),
      );
      img_activepoint.push(
        createResourceFile(
          "activePoint/activePoint_Down_LR.png",
          aP_dn,
          userDataFolder,
        ),
      );
      img_toedges.push(
        createResourceFile(
          "toEdges/toEdges_Standard_LR.png",
          tE_st,
          userDataFolder,
        ),
      );
      ...
      img_outPoint.push(
        createResourceFile(
          "outPoint/outPoint_Standard_LR.png",
          oP_st,
          userDataFolder,
        ),
      );
      img_outPoint.push(
        createResourceFile(
          "outPoint/outPoint_Hover_LR.png",
          oP_rl,
          userDataFolder,
        ),
      );
      ...
      img_delete.push(
        createResourceFile(
          "delete/delete_Deactivated_LR.png",
          de_ds,
          userDataFolder,
        ),
      );
      img_help.push(
        createResourceFile("help/help_Standard_LR.png", hp_st, userDataFolder),
      );
      img_help.push(
        createResourceFile("help/help_Hover_LR.png", hp_rl, userDataFolder),
      );
      img_help.push(
        createResourceFile("help/help_Down_LR.png", hp_dn, userDataFolder),
      );
    } else {
      img_inPoint.push(
        createResourceFile(
          "inPoint/inPoint_Standard.png",
          iP_st,
          userDataFolder,
        ),
      );
      img_inPoint.push(
        createResourceFile("inPoint/inPoint_Hover.png", iP_rl, userDataFolder),
      );
      img_inPoint.push(
        createResourceFile("inPoint/inPoint_Down.png", iP_dn, userDataFolder),
      );
      img_closegap.push(
        createResourceFile(
          "closeGap/closeGap_Standard.png",
          cG_st,
          userDataFolder,
        ),
      );
      img_closegap.push(
        createResourceFile(
          "closeGap/closeGap_Hover.png",
          cG_rl,
          userDataFolder,
        ),
      );
      img_closegap.push(
        createResourceFile("closeGap/closeGap_Down.png", cG_dn, userDataFolder),
      );
      img_delete.push(
        createResourceFile("delete/delete_Standard.png", de_st, userDataFolder),
      );
      img_delete.push(
        createResourceFile("delete/delete_Hover.png", de_rl, userDataFolder),
      );
      img_delete.push(
        createResourceFile("delete/delete_Down.png", de_dn, userDataFolder),
      );
      img_activepoint.push(
        createResourceFile(
          "activePoint/activePoint_Standard.png",
          aP_st,
          userDataFolder,
        ),
      );
      img_activepoint.push(
        createResourceFile(
          "activePoint/activePoint_Hover.png",
          aP_rl,
          userDataFolder,
        ),
      );
      img_activepoint.push(
        createResourceFile(
          "activePoint/activePoint_Down.png",
          aP_dn,
          userDataFolder,
        ),
      );
      img_toedges.push(
        createResourceFile(
          "toEdges/toEdges_Standard.png",
          tE_st,
          userDataFolder,
        ),
      );
      img_toedges.push(
        createResourceFile("toEdges/toEdges_Hover.png", tE_rl, userDataFolder),
      );
      img_toedges.push(
        createResourceFile("toEdges/toEdges_Down.png", tE_dn, userDataFolder),
      );
      img_outPoint.push(
        createResourceFile(
          "outPoint/outPoint_Standard.png",
          oP_st,
          userDataFolder,
        ),
      );
      img_outPoint.push(
        createResourceFile(
          "outPoint/outPoint_Hover.png",
          oP_rl,
          userDataFolder,
        ),
      );
      img_outPoint.push(
        createResourceFile("outPoint/outPoint_Down.png", oP_dn, userDataFolder),
      );
      img_delete.push(
        createResourceFile(
          "delete/delete_Deactivated.png",
          de_ds,
          userDataFolder,
        ),
      );
      img_help.push(
        createResourceFile("help/help_Standard.png", hp_st, userDataFolder),
      );
      img_help.push(
        createResourceFile("help/help_Hover.png", hp_rl, userDataFolder),
      );
      img_help.push(
        createResourceFile("help/help_Down.png", hp_dn, userDataFolder),
      );
    }
  }
  function closeGap(group) {
    clearOutput();
    app.beginUndoGroup("Close Gap");
    activeItem = app.project.activeItem;
    if (!(activeItem instanceof CompItem)) {
      return;
    }
    activeLyrs = activeItem.selectedLayers;
    layersAE = activeItem.layers;
    if (layersAE.length < 1) {
      return;
    }
    activeLyrs.sort(compare);
    layers = [];
    for (var i = 1; i < layersAE.length + 1; i += 1) {
      layers.push(layersAE[i]);
    }
    layers.sort(compare);
    if (!group) {
      if (activeLyrs.length < 2) {
        alert(
          "Only one layer selected, standard Close Gap function requires at least two selected layers.",
        );
        return;
      }
      for (var i = 0; i < activeLyrs.length - 1; i += 1) {
        if (activeLyrs[i].locked) {
          continue;
        }
        outPoint = activeLyrs[i].outPoint;
        secondLyr = activeLyrs[i + 1];
        inPoint = secondLyr.inPoint;
        diff = Number(inPoint) - Number(outPoint);
        if (diff < 0) {
          continue;
        }
        secondLyr.startTime = Number(secondLyr.startTime) - diff;
      }
    }
    if (group) {
      if (activeLyrs.length != 1) {
        alert("Too many layers selected, please select one.");
        return;
      }
      outPoint = activeLyrs[0].outPoint;
      forwardLyrs = [];
      for (var i = 0; i < layers.length; i += 1) {
        if (layers[i].locked) {
          continue;
        }
        if (layers[i].shy && activeItem.hideShyLayers) {
          continue;
        }
        if (layers[i].inPoint > outPoint) {
          forwardLyrs.push(layers[i]);
        }
      }
      if (forwardLyrs.length < 1) {
        return;
      }
      inPoint = forwardLyrs[0].inPoint;
      diff = Number(inPoint) - Number(outPoint);
      for (var i = 0; i < forwardLyrs.length; i += 1) {
        if (layers[i].locked) {
          continue;
        }
        if (layers[i].shy && activeItem.hideShyLayers) {
        ...
        forwardLyrs[i].startTime = Number(forwardLyrs[i].startTime) - diff;
      }
    }
    if (activeItem.time > outPoint) {
      activeItem.time = activeItem.time - diff;
    }
    if (group) {
      writeLn("Closed gap - ");
      writeLn("Layers to the right moved left to fill empty space");
    } else {
      writeLn("Closed gap - ");
      writeLn("Selected Layers moved left to fill empty space");
      writeLn("between selected layers");
    }
  }
  function deleteL() {
    clearOutput();
    ...
    selectedlyrs = activeItem.selectedLayers;
    if (selectedlyrs.length < 0) {
      return;
    }
    selectedlyrs.sort(compare);
    activeLayer = selectedlyrs[0];
    layers = activeItem.layers;
    if (layers.length < 1) {
      return;
    }
    inPoint = Number(activeLayer.inPoint);
    forwardLyrs = [];
    for (var i = 1; i <= layers.length; i += 1) {
      ...
      if (
        !includes(layers[i], selectedlyrs) &&
        Number(layers[i].inPoint) > inPoint
      ) {
        forwardLyrs.push(layers[i]);
      }
    }
    ...
    forwardLyrs.sort(compare);
    diff = Number(forwardLyrs[0].inPoint) - Number(inPoint);
    for (var i = 0; i < forwardLyrs.length; i += 1) {
      if (forwardLyrs[i].locked) {
        continue;
      }
      forwardLyrs[i].startTime = Number(forwardLyrs[i].startTime) - diff;
    }
    for (var i = 0; i < selectedlyrs.length; i += 1) {
      if (selectedlyrs[i].locked) {
        continue;
      }
      selectedlyrs[i].remove();
    }
    if (activeItem.time > inPoint) {
      var newTime = activeItem.time - diff;
      if (newTime < 0) {
        newTime = 0;
      }
      activeItem.time = newTime;
    }
    writeLn("Ripple Delete - ");
    writeLn("Selected layers removed");
    writeLn("Layers to the right moved left to fill empty space");
  }
  function activePoint(isOutPoint) {
    clearOutput();
    app.beginUndoGroup("Trim to active time");
    activeItem = app.project.activeItem;
    if (!(activeItem instanceof CompItem)) {
      return;
    }
    ...
    if (!isOutPoint) {
      tmpSearchPoint = 0;
      for (var i = 0; i < activeLyrs.length; i += 1) {
        if (activeLyrs[i].locked) {
          continue;
        }
        tmpSearchPoint = activeLyrs[i].inPoint;
        outPoint = activeLyrs[i].outPoint;
        tmpDiff = currentTime - activeLyrs[i].inPoint;
        activeLyrs[i].inPoint = currentTime;
        activeLyrs[i].outPoint = outPoint;
        if (Math.abs(tmpDiff) < Math.abs(difference)) {
          difference = tmpDiff;
        }
        if (tmpSearchPoint > searchPoint) {
          searchPoint = tmpSearchPoint;
        }
      }
      activeItem.time = activeItem.time - difference;
      for (var i = 0; i < activeLyrs.length; i += 1) {
        activeLyrs[i].startTime = Number(activeLyrs[i].startTime) - difference;
      }
    } else {
      searchPoint = 1000000;
      for (var i = 0; i < activeLyrs.length; i += 1) {
        ...
        tmpDiff = activeLyrs[i].outPoint - currentTime;
        tmpSearchPoint = activeLyrs[i].outPoint;
        activeLyrs[i].outPoint = currentTime;
        if (Math.abs(tmpDiff) < Math.abs(difference)) {
          difference = tmpDiff;
        }
        if (tmpSearchPoint < searchPoint) {
    ...
    for (var i = 1; i <= layers.length; i += 1) {
      if (layers[i].inPoint < searchPoint) {
        continue;
      }
      if (includes(layers[i], activeLyrs)) {
        continue;
      }
      if (layers[i].locked) {
        continue;
      ...
      layers[i].startTime = layers[i].startTime - difference;
    }
    if (isOutPoint) {
      writeLn("Current time - ");
      writeLn("Selected layers Out Point has been set to the current time");
      writeLn("Layers to the right moved left to fill empty space");
    } else {
      writeLn("Current time - ");
      writeLn("Selected layers In Point has been set to the current time");
      writeLn("Layers to the right moved left to fill empty space");
    }
  }
  function toedges(isComp) {
    clearOutput();
    app.beginUndoGroup("Push comp to edges");
    activeItem = app.project.activeItem;
    if (!(activeItem instanceof CompItem)) {
      return;
    }
    ...
    if (isComp) {
      ...
      outList = [];
      for (var i = 0; i < activeLyrs.length; i += 1) {
        inList.push(activeLyrs[i].inPoint);
        outList.push(activeLyrs[i].outPoint);
      }
      inList.sort();
      outList.sort();
      inPoint = inList[0];
      outPoint = outList[outList.length - 1];
      for (var i = 1; i <= layers.length; i += 1) {
        if (layers[i].locked) {
          continue;
        }
        ...
        layers[i].startTime = Number(layers[i].startTime) - inPoint;
      }
      ...
      for (var i = 0; i < activeLyrs.length; i += 1) {
        ...
        activeLyrs[i].inPoint = 0;
        activeLyrs[i].outPoint = activeItem.duration;
      }
    }
    if (isComp) {
      writeLn("Comp Size - ");
      writeLn("Set the composition start and end point to ");
      writeLn("selected layers boundaries");
    } else {
      writeLn("Comp Size - ");
      writeLn("Set selected layers start and end point to ");
      writeLn("composition boundaries (or layers limits)");
    }
  }
  function inPoint(duration, isRipple) {
    clearOutput();
    app.beginUndoGroup("Set In Point");
    var shift = duration;
    activeItem = app.project.activeItem;
    if (!(activeItem instanceof CompItem)) {
      return;
    ...
    activeLyrs.sort(compareOut);
    duration = activeItem.frameDuration * duration;
    for (var i = 0; i < activeLyrs.length; i += 1) {
      if (activeLyrs[i].locked) {
        continue;
      }
      inPoint = activeLyrs[i].inPoint;
      outPoint = activeLyrs[i].outPoint;
      lyrduration = outPoint - inPoint;
      if (lyrduration < duration) {
        duration = lyrduration;
      }
      activeLyrs[i].inPoint = inPoint + duration;
      activeLyrs[i].outPoint = outPoint;
      difference = activeLyrs[i].inPoint - inPoint;
      activeLyrs[i].startTime = Number(activeLyrs[i].startTime) - difference;
      startPoint = inPoint;
    }
    if (!isRipple) {
      if (shift < 0) {
        writeLn("In Point - ");
        writeLn("Shifted left by " + Math.abs(shift) + " frames");
      }
      if (shift > 0) {
        writeLn("In Point - ");
        writeLn("Shifted right by " + Math.abs(shift) + " frames");
      }
    }
    difference = Number(outPoint) - activeLyrs[activeLyrs.length - 1].outPoint;
    for (var i = 1; i <= layers.length; i += 1) {
      if (layers[i].locked) {
        continue;
      }
      ...
      if (layers[i].inPoint <= startPoint) {
        continue;
      }
      if (includes(layers[i], activeLyrs)) {
        continue;
      }
      layers[i].startTime = Number(layers[i].startTime) - difference;
    }
    if (activeItem.time > startPoint) {
      var newTime = activeItem.time - difference;
      if (newTime < 0) {
        newTime = 0;
      }
      activeItem.time = newTime;
    ...
    if (isRipple) {
      if (shift < 0) {
        writeLn("In Point - ");
        if (shift == 1) {
          writeLn("shifted left by " + Math.abs(shift) + " frame");
        } else {
          writeLn("shifted left by " + Math.abs(shift) + " frames");
        }
        writeLn("Layers to the right moved left to fill empty space");
      ...
      if (shift > 0) {
        writeLn("In Point - ");
        if (shift == 1) {
          writeLn("shifted right by " + Math.abs(shift) + " frame");
        } else {
          writeLn("shifted right by " + Math.abs(shift) + " frames");
        }
        writeLn("Layers to the right moved right to fill empty space");
      }
    }
  }
  function outPoint(duration, isRipple) {
    clearOutput();
    app.beginUndoGroup("Set out point");
    var shift = duration;
    activeItem = app.project.activeItem;
    if (!(activeItem instanceof CompItem)) {
      return;
    ...
    for (var i = 0; i < activeLyrs.length; i += 1) {
      ...
      outPoint = activeLyrs[i].outPoint;
      startPoint = outPoint;
      activeLyrs[i].outPoint = outPoint - duration;
    }
    if (!isRipple) {
      if (shift < 0) {
        writeLn("Out Point - ");
    ...
    difference =
      Number(startPoint) - activeLyrs[activeLyrs.length - 1].outPoint;
    for (var i = 1; i <= layers.length; i += 1) {
      if (layers[i].locked) {
        continue;
      }
      ...
      if (layers[i].outPoint <= startPoint) {
        continue;
      }
      if (includes(layers[i], activeLyrs)) {
        continue;
  ...
  var af_settings = {
    helpText: [
      "Copyright (\xa9) 2021 Plugin Everything",
      "",
      "Ripple Edit is a tool built for After Effects that brings advanced editing capabilities into your comps.",
      "Ripple Edit consists of 6 buttons, each button has a standard function with most having a shift and/or alt modifier that can be activated by holding the respective key on the keyboard. The modifiers (if the button has them) can be used together (shift+alt) to combine their effects.",
      "The buttons by default are stacked vertically, holding alt and double clicking on the toolbar will rotate the orientation of the buttons, swapping between horizontal and vertical. This orientation is saved between instances of After Effects.",
      "From top to bottom (or left to right) the buttons are are:",
      "",
      "In Point",
      ...
      "    Holding Shift will do the same, but shift by 10 frames instead.",
      "    Holding Alt will push the in point out instead of in, shifting the layer right instead of left.",
      "",
      "Close Gap",
      ...
      "    Holding Alt will set the out point instead of the in point of the layer.",
      "",
      "To Comp",
      ...
      "",
      "Out Point",
      "    This is almost identical to In Point except it works on the out point of selected layer(s), pulling the out point left by one, shifting all layers to the right.",
      "    Holding Shift will do the same, but pull by 10 frames instead.",
      "    Holding Alt will push the out point out instead of in, shifting layers to the right instead of left.",
      "",
      "Help",
      "    This gear shaped button will open the help and licensing menu.",
    ].join("\n"),
    offerTrial: true,
    privateNumber: 7742661513484355,
    productSKU: "PERE-SUL",
    scriptAuthor: "Plugin Everything",
    scriptName: "Ripple Edit",
    scriptURL: "http://aescripts.com/ripple-edit/",
    scriptVersion: "1.1.3",
  };
  function a(vars) {
    function licUI() {
    ...
    var maxUIButtons = 3;
    var licV = 2;
    var mx = __BLOB__BLOB_000665__;
    var wx = __BLOB__BLOB_000666__;
  ...
  var MAIN_JSX = "main.jsx";
  var Exception = (function Exception() {
    function hexToCol(hexString) {
    ...
    function calculateMultilineHeight(controlObj) {
      var text = controlObj.text;
      var width = controlObj.size.width;
      var measurement = controlObj.graphics.measureString(
        text,
        undefined,
        width,
      );
    ...
    function log(error, fileName) {
      try {
        if (error.stack === undefined) {
          ...
          if (stack[stack.length - 1].substr(0, 3) === "log") {
            stack.pop();
          }
          if (stack[stack.length - 1].substr(0, 8) === 'create("') {
            stack.pop();
          }
          error.stack = stack;
        }
        error.fileTrace = error.fileTrace || fileName;
        return error;
      } catch (err) {
        alert(err);
      }
    }
    function create(message, fileName, line, name) {
      try {
        var err = new Error(message, fileName, line);
        err.name = name || "Exception";
        return log(err, fileName);
      } catch (err) {
        alert(err);
      }
    }
    function report(error) {
      try {
        ...
        win.onResizing = win.onResize = function () {
          this.layout.resize();
          this.statictext.size.height = calculateMultilineHeight(
            this.statictext,
          );
          this.grp_errorInfo.grp_title.func.size.height =
            calculateMultilineHeight(this.grp_errorInfo.grp_title.func);
        };
        win.onShow = function () {
          this.onResizing();
        ...
        win.seperator.onDraw = function () {
          this.graphics.rectPath(0, 0, this.size[0], 1);
          this.graphics.fillPath(fillBrush);
        };
        win.grp_errorInfo.graphics.backgroundColor = graphics.newBrush(
          graphics.BrushType.SOLID_COLOR,
          hexToCol("#242424"),
        );
        win.grp_errorInfo.grp_title.func.text =
          error.name + ": " + error.message;
        win.grp_errorInfo.grp_title.file.text =
          "[" + error.fileTrace ||
          File(error.fileName).name + ":" + error.line + "]";
        var stack = error.stack || [];
        var group = win.grp_errorInfo;
        for (var i = 0; i < stack.length; i += 1) {
          group.add(StaticText({ text: "    " + stack[i] }));
        }
        win.show();
      } catch (err) {
        alert(err);
    ...
    var resReport = Dialog(
      ...
      {
        btn_ok: Button({ alignment: ["right", "bottom"], text: "Ok" }),
        grp_errorInfo: Group(
          {
          ...
          },
          {
            ...
            grp_os: Group(
              { orientation: "row" },
              {
                ...
                file: StaticText({ alignment: ["right", "top"], text: $.os }),
              },
            ),
          },
        ),
        seperator: Panel({ maximumSize: [100000, 1], preferredSize: [-1, 1] }),
        statictext: StaticText({
          alignment: ["fill", "top"],
          maximumSize: [100000, 450],
          properties: { multiline: true },
          text: [
            "We\'re sorry, something has gone wrong with Ripple Edit that we didn\'t account for",
            "In order to help us fix this for you, and prevent others from having the same issue, we ask that you submit a support ticket to use via aescripts.com",
            "Please include details about your computer and the steps that led up to this error, a screenshot of this error would also be very helpful",
            "This will help us get you up and running again as soon as possible",
          ].join("\\n"),
        }),
      },
    );
    return { create: create, log: log, report: report };
  })();
})(this);
