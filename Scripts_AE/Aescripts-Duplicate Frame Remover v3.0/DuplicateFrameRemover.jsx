...
function la_DuplicateFrameRemover(thisObj) {
  function buildUI(thisObj) {
    ...
    if (myPal != null) {
      var res =
        "group { \n\t\t\t\t\torientation:\'column\', alignment:[\'left\',\'fill\'], alignChildren:[\'left\',\'top\'], spacing:5, margins:[0,0,0,0], \n\t\t\t\t\tthreshold: Group { \n\t\t\t\t\t\talignment:[\'left\',\'top\'], alignChildren:[\'left\',\'top\'], \n\t\t\t\t\t\tlbl: StaticText { text:\'Threshold\', preferredSize: [70,20] }, \n\t\t\t\t\t\tsldr: Slider { minvalue:0, maxvalue:5, preferredSize: [128,20], }, \n\t\t\t\t\t\tval: StaticText{ text:\'" +
        thresholdDefault +
        "\' }, \n\t\t\t\t\t}, \n\t\t\t\t\troi: Group { \n\t\t\t\t\t\talignment:[\'left\',\'top\'], alignChildren:[\'left\',\'top\'], \n\t\t\t\t\t\tcheck: Checkbox { text:\'Use Region of Interest\'}, \n\t\t\t\t\t\tbtn: Button { text:\'Create ROI layer\', preferredSize: [-1,25]}, \n\t\t\t\t\t}, \n\t\t\t\t\tretime: Group { \n\t\t\t\t\t\talignment:[\'left\',\'top\'], alignChildren:[\'left\',\'top\'], \n\t\t\t\t\t\tcheck: Checkbox { text:\'Retime to original length\', helpTip: \'Removing frames will make the footage shorter, this option will make the time-remap keyframes linear and time-stretch it back to the original length using After Effects frame interpolation\'}, \n\t\t\t\t\t\tkind: DropDownList { properties:{items:[\'No Blending\',\'Frame Mix\',\'Pixel Motion\']} }, \n\t\t\t\t\t}, \n\t\t\t\t\tprogress: Group { \n\t\t\t\t\t\talignment:[\'fill\',\'top\'], alignChildren:[\'fill\',\'top\'], \n\t\t\t\t\t\tval: Progressbar {  }, \n\t\t\t\t\t}, \n\t\t\t\t\tbtns: Group { \n\t\t\t\t\t\talignment:[\'fill\',\'top\'], alignChildren:[\'right\',\'top\'], \n\t\t\t\t\t\thelp: Button { text:\'?\', alignment:[\'left\',\'top\'], preferredSize: [20,20] }, \n\t\t\t\t\t\tdoItBtn: Button { text: \'Do It!\', alignment:[\'right\',\'top\'], preferredSize: [60,25] }, \n\t\t\t\t\t\t},\n\t\t\t\t}";
      ...
      myPal.grp.threshold.sldr.helpTip =
        "Keep this value slightly above zero to let compression and noise not be considered enough difference for a frame to be unique. IF YOUR FOOTAGE IS HIGHLY COMPRESSED OR NOISY RAISE THE LEVEL";
      myPal.grp.threshold.sldr.value = app.settings.haveSetting(
        "DuplicateFrameRemover",
        "Threshold",
      )
        ? parseFloat(
            app.settings.getSetting("DuplicateFrameRemover", "Threshold"),
          )
        : thresholdDefault;
      myPal.grp.roi.check.value = app.settings.haveSetting(
        "DuplicateFrameRemover",
        "ROI",
      )
        ? !(app.settings.getSetting("DuplicateFrameRemover", "ROI") == "false")
        : useRegionOfInterestDefault;
      myPal.grp.roi.btn.visible = myPal.grp.roi.check.value;
      myPal.grp.threshold.val.text = myPal.grp.threshold.sldr.value;
      myPal.grp.retime.check.value = app.settings.haveSetting(
        "DuplicateFrameRemover",
        "Retime",
      )
        ? !(
            app.settings.getSetting("DuplicateFrameRemover", "Retime") ==
            "false"
          )
        : false;
      myPal.grp.retime.kind.visible = myPal.grp.retime.check.value;
      myPal.grp.retime.kind.selection = app.settings.haveSetting(
        "DuplicateFrameRemover",
        "RetimeKind",
      )
        ? parseInt(
            app.settings.getSetting("DuplicateFrameRemover", "RetimeKind"),
            10,
          )
        : 2;
      myPal.grp.progress.hide();
      myPal.grp.threshold.sldr.onChanging = function () {
        this.parent.parent.threshold.val.text =
          this.parent.parent.threshold.sldr.value;
      };
      myPal.grp.roi.btn.onClick = function () {
        addROI();
      };
      myPal.grp.btns.help.onClick = function () {
        df.helpUI();
      };
      myPal.grp.threshold.sldr.onChange = function () {
      ...
      myPal.grp.roi.check.onClick = function () {
        myPal.grp.roi.btn.visible = this.value;
        app.settings.saveSetting("DuplicateFrameRemover", "ROI", this.value);
      };
      myPal.grp.retime.check.onClick = function () {
        myPal.grp.retime.kind.visible = this.value;
        app.settings.saveSetting("DuplicateFrameRemover", "Retime", this.value);
      };
      myPal.grp.retime.kind.onChange = function () {
        app.settings.saveSetting(
          "DuplicateFrameRemover",
          "RetimeKind",
      ...
      myPal.grp.btns.doItBtn.onClick = function () {
        onDoItClick(myPal);
      };
    }
    return myPal;
  }
  function doTheWork(palObj, selItem) {
    var threshold = palObj.grp.threshold.sldr.value;
    var progress = palObj.grp.progress.val;
    var useRegionOfInterest = palObj.grp.roi.check.value;
    var retime = palObj.grp.retime.check.value;
    var retimeKind = palObj.grp.retime.kind.selection.index;
    ...
    if (useRegionOfInterest) {
      if (myComp.layer(roiLayerName) == null) {
        alert(
          "Could not find Region of Interest Locator, please re-run script to create it",
        );
        return;
      } else {
        var roiLayer = myComp.layer("DFR-Region of Interest");
        var roiCenter = roiLayer
          .property("ADBE Effect Parade")
          .property("ADBE ELLIPSE")
          .property("ADBE ELLIPSE-0001").value;
        var roiWidth = roiLayer
          .property("ADBE Effect Parade")
          .property("ADBE ELLIPSE")
          .property("ADBE ELLIPSE-0002").value;
        var roiHeight = roiLayer
          .property("ADBE Effect Parade")
          .property("ADBE ELLIPSE")
          .property("ADBE ELLIPSE-0003").value;
      }
    } else {
      var roiCenter = ["width/2", "height/2"];
      var roiWidth = "width";
      var roiHeight = "height";
    }
    var myLayer = selItem;
    var myIndex = myLayer.index;
    myLayer = myComp.layer(myIndex);
    myLayer.duplicate();
    var myPreComp = myComp.layers.precompose(
      [myIndex + 1],
      "_Magnum_at_Work_",
      false,
    );
    ...
    myDupLayer.startTime = oneFrame * -1;
    var myEffect = myComp
      .layer(myIndex + 1)
      .property("ADBE Effect Parade")
      .addProperty("ADBE Shift Channels");
    myEffect.property("ADBE Shift Channels-0001").setValue(5);
    var myThresEffect = myComp
      .layer(myIndex + 1)
      .property("ADBE Effect Parade")
      .addProperty("ADBE Slider Control");
    myThresEffect.property("ADBE Slider Control-0001").setValue(threshold);
    var myExpEffect = myComp
      .layer(myIndex + 1)
      .property("ADBE Effect Parade")
      .addProperty("ADBE Slider Control");
    myExpEffect.property("ADBE Slider Control-0001").expression =
      "p=[" +
      roiCenter[0] +
      "," +
      roiCenter[1] +
      ...
      "s=sampleImage(p, radius = [" +
      roiWidth +
      "," +
      roiHeight +
      "], postEffect = true, t = time); \n" +
      "s[3]*100";
    var savedOutPoint = Math.min(myLayer.outPoint, myComp.workAreaDuration);
    var myWorkAreaInFrames = savedOutPoint * myComp.frameRate;
    var savedInPoint = Math.max(myLayer.inPoint, myComp.workAreaStart);
    var myWorkAreaStartInFrames = savedInPoint * myComp.frameRate;
    if (isTrial) {
      myWorkAreaInFrames = Math.min(myWorkAreaInFrames, 160);
    }
    var magnum = myComp.layer(myIndex + 1);
    var myOrigLayer = myComp.layer(myIndex);
    var found = 0;
    var keyType = KeyframeInterpolationType.HOLD;
    var goodFrames = new Array();
    progress.value = 0;
    progress.maxvalue = myWorkAreaInFrames - 1;
    palObj.grp.progress.show();
    for (var i = 1; i < myWorkAreaInFrames; i += 1) {
      progress.value++;
      ...
      var myResult = magnum
        ...
        .property(3)
        .property(1)
        .valueAtTime(myTime, false);
      writeLn(
        "Processing frame " + i + " out of " + Math.round(myWorkAreaInFrames),
      );
      clearOutput();
      if (myResult > threshold) {
        goodFrames[found] = myTime + myOrigLayer.startTime * -1;
        found++;
      }
    }
    myPreComp.remove();
    var newLayer = myComp.layer(myIndex);
    newLayer.timeRemapEnabled = true;
    var myOutPoint = goodFrames.length / myComp.frameRate + savedInPoint;
    var changeRate = savedOutPoint / myOutPoint;
    var myTimes = [];
    for (var t = 0; t < goodFrames.length; t += 1) {
      myTime2 = retime ? t * changeRate : t;
      myTimes.push((myWorkAreaStartInFrames + myTime2) / myComp.frameRate);
    }
    newLayer.timeRemap.setValuesAtTimes(myTimes, goodFrames);
    if (!retime) {
      for (var k = 0; k < goodFrames.length; k += 1) {
        newLayer.timeRemap.setInterpolationTypeAtKey(k + 1, keyType);
      }
    } else {
      myComp.frameBlending = true;
      switch (retimeKind) {
        case 0:
          newLayer.frameBlendingType = FrameBlendingType.NO_FRAME_BLEND;
          break;
        case 1:
          newLayer.frameBlendingType = FrameBlendingType.FRAME_MIX;
          break;
        case 2:
          newLayer.frameBlendingType = FrameBlendingType.PIXEL_MOTION;
          break;
      }
    }
    newLayer.inPoint = savedInPoint;
    newLayer.outPoint = myOutPoint + 1 * myComp.frameRate;
    var originalFrames = Math.round(myWorkAreaInFrames);
    var removedFrames = originalFrames - goodFrames.length;
    palObj.grp.progress.hide();
    app.endUndoGroup();
    if (useRegionOfInterest) {
      var removeRoi = confirm(
        "Done!\n Removed " +
          removedFrames +
          " duplicate frames.\n\nWould you like to delete the Region Of Interest layer?",
      );
      if (removeRoi) {
        app.beginUndoGroup(scriptTitle + "-Remove Region of Interest layer");
        roiLayer.remove();
        app.endUndoGroup();
      }
    } else {
      if (removedFrames >= originalFrames - 1) {
        alert(
          "All frames were detected to be duplicates which probably means you have the threshold set too high. Please undo and try again at a lower threshold setting. You can also use the Region of Interest feature to help guide the detection.",
        );
      } else {
        alert("Done!\n Removed " + removedFrames + " duplicate frames.");
      }
    }
  }
  function addROI(selItem) {
    if (selItem == null) {
      selItem = preFlight();
      if (selItem == false) {
        return;
    ...
    if (myComp.layer(roiLayerName) == null) {
      app.beginUndoGroup(scriptTitle + "-Create Region of Interest");
      var roiLayer = myComp.layers.addSolid(
        [0, 0, 0],
        roiLayerName,
        selItem.source.width,
        selItem.source.height,
        selItem.source.pixelAspect,
        selItem.source.duration,
      );
      roiLayer.parent = selItem;
      var ellipseEffect = roiLayer
        .property("Effects")
        .addProperty("ADBE ELLIPSE");
      ellipseEffect.name = "Set Region of Interest Center and Width";
      ellipseEffect.property("ADBE ELLIPSE-0002").setValue(100);
      ellipseEffect.property("ADBE ELLIPSE-0003").expression =
        'thisLayer.effect("ADBE ELLIPSE")("ADBE ELLIPSE-0002")';
      roiLayer.guideLayer = true;
      roiLayer.startTime = selItem.startTime;
      roiLayer.outPoint = selItem.outPoint;
      app.endUndoGroup();
      alert(
        "Before we move forward, the Region of Interest needs to be defined\nReveal the effects for the Region of Interest layer that was just created and set the center, width and height to where you would like to detect changes.  Please note that these values can be keyframed.\n\nOnce you\'ve done this, re-select the layer and click the Do It! button again",
      );
      return true;
    } else {
      return false;
  ...
  function preFlight() {
    ...
    if (comp == null || !(comp instanceof CompItem)) {
      alert(
        "Please select the layer you would like to remove the duplicate frames from and make sure the containing comp is active (it should have a yellow outline)",
      );
      return false;
    }
    if (comp.selectedLayers.length == 0) {
      alert(
        "Please select the layer you would like to remove the duplicate frames from",
      );
      return false;
    }
    var selItem = comp.selectedLayers;
    if (selItem.length > 1) {
      alert("You can only have 1 layer selected");
      return false;
    }
    if (selItem[0].source.file == null && !(comp instanceof CompItem)) {
      alert("Selected layer can\'t be a light, solid, null, etc");
      return false;
    }
    if (selItem[0].stretch != 100) {
      alert(
        "The layer can\'t be stretched, please set stretch to 100% or pre-comp it",
      );
      return false;
    }
    if (selItem[0].name == roiLayerName) {
      alert(
        "Please select the layer you would like to detect duplicate frames on instead of the ROI layer",
      );
      return false;
    }
    if (selItem[0].timeRemapEnabled == true) {
      alert(
        "Layer cannot have time remapping enabled, please pre-comp and try again",
      );
      return false;
    }
    return selItem[0];
  }
  function onDoItClick(palObj) {
    var selItem = preFlight();
    if (selItem == false) {
      return;
    }
    var useRegionOfInterest = palObj.grp.roi.check.value;
    if (useRegionOfInterest) {
      var createdRoi = addROI(selItem);
      if (createdRoi) {
        return;
      }
    }
    app.beginUndoGroup("Duplicate Frame Remover");
    ...
    var goAheadTxt =
      "Notice\n\nDepending on the computer speed and the size and\nlength of the footage, this might take a while.\n\n";
    if (parseFloat(app.version) < 9) {
      goAheadTxt +=
        "Please open the Info Panel (" +
        cmd +
        "2)\nto monitor the progress.\n\n";
    }
    goAheadTxt +=
      "Unfortunately the process can\'t be canceled once it has started.\n\nWould you like to continue?";
    var goAhead = confirm(goAheadTxt);
    if (goAhead) {
      doTheWork(palObj, selItem);
    }
    app.endUndoGroup();
  }
  var useRegionOfInterestDefault = false;
  var thresholdDefault = 0.3;
  var scriptTitle = "Duplicate Frame Remover";
  var scriptVersion = "3.0";
  var roiLayerName = "DFR-Region of Interest";
  var strHelp =
    'Automatically detects duplicate frames in footage. \n\nINSTRUCTIONS:\nWorks by comparing sequential frames.  To improve results or if youe footage changes in only part of the frame, we highly recommend that you use the "Use Region of Interest" option to define an area of the footage to concentrate on.  To use the "Use Region of Interest" option you first need to create an ROI layer. This layer contains an ellipse effect that you will use to define the region that should be scanned for changes so that it can determine if a frame is a duplicate or not.  Once you\'ve set the Region of Interest, select your footage layer again and press Do It one more time to remove the duplicate frames.\n\nIf your footage is noise free then set the threshold to 0 for best results.\n\n';
  ('Removing frames will make the footage shorter, the "Retime to original lenght" option will make the time-remap keyframes linear and time-stretch it back to the original length using After Effects frame interpolation');
  var af_settings = {
    betaExpirationDate: new Date("Mar 31, 2020"),
    betaStartDate: new Date("Feb 6, 2020"),
    helpButtons: [],
    helpText: strHelp,
    offerBeta: false,
    offerTrial: true,
    privateNumber: 6339256492872290,
    productSKU: "LADFR3-SUL",
    scriptAuthor: "Lloyd Alvarez",
    scriptName: scriptTitle,
    scriptURL: "https://aescripts.com/duplicate-frame-remover/",
  ...
  function a(vars) {
    ...
    var maxUIButtons = 3;
    var licV = 2;
    var wx = __BLOB__BLOB_000044__;
    var mx = __BLOB__BLOB_000045__;
  ...
  var df = new a(af_settings);
  if (df.c()) {
    var isTrial = df.vt();
    if (isTrial) {
      alert("Please note that trial mode is limited to processing 150 frames");
    }
...
if (parseFloat(app.version) >= 16.1) {
  la_DuplicateFrameRemover(this);
} else {
  alert("After Effects version 16.1 of higher is required to use this tool");
}
