...
function oc_Trails_script(thisObj) {
  var af_settings = {
    ...
    helpButtons: [
      {
        ...
        url: "http://www.othercubed.com/trails-help",
      },
      {
        name: "Other Products",
        url: "https://aescripts.com/authors/m-p/othercubed/",
      },
    ],
    helpText:
      " Thanks for using Trails! For full documentation please press \'view online documentation\' below.\n    \n    \n Basic instructions:\n    \n - Hit \'Create Trails Layer\' to create a new trails layer. If you want to parent your first point to another layer straight away, select that layer and hold shift while you\'re creating your new Trails layer! \n    \n - Hit the \'+\' button to add a new point to your trails layer (make sure you have a trails layer selected). If you want to parent to another layer straight away, select your Trails layer and THEN the layer you want to parent to before you hit \'+\'!\n    \n - Hit the \'-\' button while you have your Trails layer selected to delete the last added trail. If you\'ve changed which layer controls your point, you\'ll get a popup asking if you want to delete that layer when you delete your trail! \n    \n    \n    \n Top tips:\n    \n - The point samples will increase the number of points on the Trail path created. A higher number means more calculations are being done, so careful going too high!\n    \n - Point samples are cumulative - so if you have 4 points in your Trails layer, and your point samples are set to 50, there will be 200 calculations being done!\n    \n - A good way to get smoother lines is to select your Trail paths in the Appearance group, go to Layer>Mask and Shape Path, and check \'RotoBezier\' to on. Unfortunately this can only currently be set by the user!\n    ",
    offerBeta: false,
    offerTrial: true,
    privateNumber: 6437758441884286,
    productSKU: "OCT-SUL",
    scriptAuthor: "OtherCubed",
    scriptName: "Trails",
    scriptURL: "http://aescripts.com/trails",
    scriptVersion: "1.0",
  };
  function a(vars) {
    function licUI() {
    ...
    var maxUIButtons = 3;
    var licV = 2;
    var wx = __BLOB__BLOB_000547__;
    var mx = __BLOB__BLOB_000548__;
  ...
  var ocTa = new a(af_settings);
  if (ocTa.c()) {
    function trailsMain(thisObj) {
      function scriptBuildUI(thisObj) {
        ...
        var trailsUI =
          thisObj instanceof Panel
            ? thisObj
            : new Window("palette", "OC Trails", undefined, {
                resizeable: true,
              });
        var createNew = trailsUI.add(
          "iconbutton",
          undefined,
          trailsButtonIcon,
          { style: "toolbutton" },
        );
        createNew.alignment = ["fill", "top"];
        var smallButtonGroup = trailsUI.add("group");
        smallButtonGroup.alignment = ["fill", "bottom"];
        smallButtonGroup.alignChildren = ["fill", "fill"];
        smallButtonGroup.spacing = 5;
        var newPoint = smallButtonGroup.add("button", undefined, "+");
        newPoint.preferredSize.width = 0;
        var removePoint = smallButtonGroup.add("button", undefined, "-");
        removePoint.preferredSize.width = 0;
        var helpButton = smallButtonGroup.add("button", undefined, "?");
        helpButton.preferredSize.width = 0;
        createNew.helpTip =
          "Create a new Trails Layer and point.\n \nHold down shift to parent the first point to the currently selected layer.";
        newPoint.helpTip =
          "Add a new point to the selected Trails Layer.\n\nSelect your trails layer and then a secondary layer to parent the new point to the secondary layer.";
        removePoint.helpTip =
          "Remove the last point from the selected Trails Layer";
        helpButton.helpTip = "See more help and information about Trails";
        createNew.onClick = function () {
          app.beginUndoGroup("Create new Trails Layer");
          var shiftClicked = shiftState();
          var selectedLayer = 0;
          createLayer(shiftClicked);
          app.endUndoGroup();
        };
        newPoint.onClick = function () {
          app.beginUndoGroup("Create new Trails Point");
          var selectedLayer = 0;
          createPoint();
          app.endUndoGroup();
        };
        removePoint.onClick = function () {
          app.beginUndoGroup("Remove Trails Point");
          var selectedLayer = 0;
          removeTrailPoint();
          app.endUndoGroup();
        };
      ...
      function createLayer(shiftClicked) {
        var isTrial = ocTa.t();
        var trailLayerLimit = 1;
        isTrial = !ocTa.s();
        var curComp = app.project.activeItem;
        if (curComp == null) {
          alert("No comp currently active!");
          return;
        }
        var numControls = findTrailControllers();
        if (isTrial && numControls > trailLayerLimit) {
          alert(
            "The trial version is limited to " +
              trailLayerLimit +
              " Trails layers per comp. Sorry!",
          );
          return;
        }
        var layWhenClicked = curComp.selectedLayers;
        var trailLayer = curComp.layers.addShape();
        trailLayer.comment = "OC TRAILS CONTROL " + numControls;
        trailLayer.name = "OC Trails " + numControls;
        var appGroup = trailLayer
          .property("ADBE Root Vectors Group")
          .addProperty("ADBE Vector Group");
        appGroup.name = appGroupName;
        var appPathGroup = appGroup
          .property("ADBE Vectors Group")
          .addProperty("ADBE Vector Group");
        appPathGroup.name = appPathGroupName;
        var appGroupStroke = appGroup
          .property("ADBE Vectors Group")
          .addProperty("ADBE Vector Graphic - Stroke");
        appGroupStroke.property("ADBE Vector Stroke Width").setValue("10");
        appGroupStroke.property("ADBE Vector Stroke Line Cap").setValue(2);
        appGroupStroke.property("ADBE Vector Stroke Line Join").setValue(2);
        var mathGroup = trailLayer
          .property("ADBE Root Vectors Group")
          .addProperty("ADBE Vector Group");
        mathGroup.name = mathsGroupName;
        if (layWhenClicked.length > 0) {
          trailLayer.moveBefore(layWhenClicked[0]);
        }
        var checkShift = 0;
        if (shiftClicked > 0) {
          checkShift = layWhenClicked[0].index;
        }
        createPoint(checkShift);
        trailLayer.selected = true;
        applyTrailsPseudo();
      }
      function removeTrailPoint() {
        ...
        if (curComp == null) {
          alert(
            "Whoops! No comp currently active. Please select a Trails layer to remove a point from",
          );
          return;
        }
        var laySelected = curComp.selectedLayers;
        var pointArray = [];
        if (
          laySelected.length == 1 &&
          laySelected[0].comment.indexOf("OC TRAILS CONTROL") >= 0
        ) {
          function handleRemovePaths(g) {
            if (g.name.indexOf("Point " + pointToRemove + " Trail") >= 0) {
              g.remove();
            } else if (
              g.name.indexOf("Trail " + pointToRemove + " Extra Calcs") >= 0
            ) {
              g.remove();
            } else {
              if (g.name.indexOf("Trail Base Path " + pointToRemove) >= 0) {
                g.remove();
              }
            }
          }
          function scoutContent(g, handler) {
            ...
            for (n = g.numProperties; n > 0; n--) {
              child = g.property(n);
              handler(child);
              if (Object.isValid(child) && child.content) {
                scoutContent(child, handler);
              }
            }
          }
          function removeControlLayer(selectedLayer, pointNum) {
            var controlToRemove = selectedLayer
              .property("ADBE Effect Parade")
              .property("Point " + pointNum + " Control Layer");
            var layIndexToRemove = controlToRemove.property(1).value;
            var layToRemove = curComp.layer(layIndexToRemove);
            controlToRemove.remove();
            if (layToRemove.comment.indexOf("POINT " + pointNum) >= 0) {
              layToRemove.remove();
            } else {
              var nonPointLayerAlert = confirm(
                "This point is currently controlled by layer " +
                  layToRemove.name +
                  ". Do you want to delete this control layer?",
              );
              if (nonPointLayerAlert == true) {
                layToRemove.remove();
              }
            }
          }
          var thisController = laySelected[0].comment;
          var controlNum =
            "OCT" + thisController.substring(thisController.length - 1);
          var checkBasePaths = laySelected[0]
            .property("ADBE Root Vectors Group")
            .property(mathsGroupName)
            .property("ADBE Vectors Group");
          for (var i = 1; i < checkBasePaths.numProperties; i += 1) {
            var iProperty = checkBasePaths.property(i).name;
            if (iProperty.indexOf("Trail Base Path ") >= 0) {
              pointArray.push(iProperty.substring(iProperty.length - 1));
            }
          }
          var pointToRemove = Math.max.apply(null, pointArray);
          if (pointToRemove >= 2) {
            scoutContent(laySelected[0], handleRemovePaths);
            removeControlLayer(laySelected[0], pointToRemove);
          } else if (pointToRemove == "-Infinity") {
            alert("No Points left to delete!");
          } else {
            var tooFewPoints = confirm(
              "Only one Point left! Are you sure you want to delete it?",
              false,
            );
            if (tooFewPoints == true) {
              scoutContent(laySelected[0], handleRemovePaths);
              removeControlLayer(laySelected[0], pointToRemove);
            }
          }
        } else {
          alert("Please select a Trails layer to remove a point from");
        }
      }
      function findTrailControllers() {
        var curComp = app.project.activeItem;
        var l = 1;
        for (var i = 1; i < curComp.numLayers; i += 1) {
          if (curComp.layer(i).comment.indexOf("OC TRAILS CONTROL") >= 0) {
            l += 1;
          }
        }
        return l;
      }
      function addNewShapePaths(controlLayer, pointName) {
        var conLayerContents = controlLayer.property("ADBE Root Vectors Group");
        var baseGroup = conLayerContents
          .property(mathsGroupName)
          .property("ADBE Vectors Group");
        var newBaseShape = baseGroup.addProperty("ADBE Vector Shape - Group");
        newBaseShape.name = "Trail Base Path " + pointName;
        newBaseShape.path.expression = mainCode(pointName);
        var BaseShapeName = newBaseShape.name;
        var newAppShape = conLayerContents
          .property(appGroupName)
          .property("ADBE Vectors Group")
          .property(appPathGroupName)
          .property("ADBE Vectors Group")
          .addProperty("ADBE Vector Shape - Group");
        newAppShape.name = "Point " + pointName + " Trail";
        newAppShape.path.expression =
          '//OC Trails - leave this code be! \n \n thisLayer.content("' +
          mathsGroupName +
          '").content("' +
          BaseShapeName +
          '").path';
        var newCalcGroup = baseGroup.addProperty("ADBE Vector Group");
        newCalcGroup.name = "Trail " + pointName + " Extra Calcs ";
        newCalcGroup
          .property("ADBE Vector Transform Group")
          .property(1).expression = extraCode(pointName);
      }
      function mainCode(inputLayer) {
        var conCode =
          "//OC TRAILS CODE - DON\'T TOUCH!\n\n//base sliders\np = effect(\"Point " +
          inputLayer +
          ' Control Layer")(1);\n\n//velocityChangeLayer\ncurVel = content("LEAVE ME ALONE").content("Trail 1 Extra Calcs ").transform.anchorPoint[0];\n\nsP = effect("OC Trails")("Point Samples");\ntL = (effect("OC Trails")("Trail Length")*-1)/10;\n\n//checkboxes\nwigCheck = effect("OC Trails")("Wiggler");\nvelCheck = effect("OC Trails")("Velocity Trails");\nmovCheck = effect("OC Trails")("Add Movement");\nvelLength = effect("OC Trails")("Velocity Based Line Length");\n\n//wiggle sliders\nw1 = effect("OC Trails")("Wiggles/Second");\nw2 =effect("OC Trails")("Wiggle Amount");\nwC = effect("OC Trails")("Wiggle Cohesion");\nwS = effect("OC Trails")("Wiggle at Start");\nwE = effect("OC Trails")("Wiggle at End");\nwigCheckX = effect("OC Trails")("Wiggle X");\nwigCheckY = effect("OC Trails")("Wiggle Y");\n\n//velocity sliders\nvelA = effect("OC Trails")("Velocity Amount");\nvelS = effect("OC Trails")("Velocity at Start");\nvelE = effect("OC Trails")("Velocity at End");\nvelCheckX = effect("OC Trails")("Velocity X");\nvelCheckY = effect("OC Trails")("Velocity Y");\n\n//movement sliders\nmovD = effect("OC Trails")("Movement Direction");\nmovS = effect("OC Trails")("Movement at Start");\nmovE = effect("OC Trails")("Movement at End");\n\n//velocity vars\nv = 0;\nf = 30; //frames to average\nfD = thisComp.frameDuration;\n\nposPoints = [];\n\noff = tL/sP;\n\n\t//main application\n\nfor(var i = 0; i < sP; i++){\n\n\npCalc = trailPos(p);\npOrig = p.toWorld([0,0,0]);\npMod = pCalc;\n\nif(wigCheck == 1){\npMod = pMod + wiggler(p,i);\n}\nif(velCheck == 1){\npMod = pMod + velMod(p);\n}\nif(movCheck == 1){\npMod = pMod + movMod(p,i);\n}\n\npVel = linear(curVel,0,100,pCalc,pMod)\n\npPoint = (pVel) - thisLayer.toComp([0,0,0]);\nposPoints.push(pPoint);\n}\n\n\n\t//functions\n\nfunction StEndStrength(inSlid,outSlid,maxVal){\n\tsT = linear(inSlid,-100,100,maxVal*-1,maxVal);\n\teT = linear(outSlid,-100,100,maxVal*-1,maxVal);\nreturn [sT,eT];\n}\n\nfunction trailPos(x){\n\ttP = x.toComp(x.transform.anchorPoint,time + (off*i));\nreturn [tP[0],tP[1]];\n}\n\nfunction wiggler(pPos, x){\n\tCo = linear(wC,0,100,1,100);\n\tw = pPos.transform.position.wiggle(w1,w2,1,0.5,time-(x/Co)) - pPos.transform.position;\n\tminMax = StEndStrength(wS,wE,w);\n\twig = linear(x,0,sP,minMax[0],minMax[1]);\n\nif(wigCheckX == 1){\nwigX = wig[0];\n}else{\nwigX = 0;\n}\nif(wigCheckY == 1){\nwigY = wig[1];\n}else{\nwigY = 0;\n}\n\nwFinal = [wigX,wigY];\n\nreturn wFinal;\n}\n\n\nfunction velMod(x){\n\tvP1 = x.toWorld([0,0,0],(time + (off*i))+thisComp.frameDuration*2);\n\tvP2 = x.toWorld([0,0,0],(time + (off*i))-thisComp.frameDuration*2);\n\tv = (((vP1 - vP2)/thisComp.frameDuration)/15000)*(velA*5);\n\tminMax = StEndStrength(velS,velE,v);\n\tvE = linear(i,velS,sP,minMax[0],minMax[1]);\n\nif(velCheckX == 1){\nvelX = vE[0];\n}else{\nvelX = 0;\n}\nif(velCheckY == 1){\nvelY = vE[1];\n}else{\nvelY = 0;\n}\n\nvelFinal = [velX,velY];\nreturn (velFinal);\n}\n\nfunction movMod(pPos, x){\n\ttMod = movD;\n\tminMax = StEndStrength(movS,movE,tMod);\n\tmMod = linear(x,0,sP,minMax[0],minMax[1]);\nreturn mMod;\n}\n\n\n//find Velocity Length\n\nfunction timeVel (o,p){\nvP1 = p.toWorld([0,0,0],(time+o)+thisComp.frameDuration);\n\tvP2 = p.toWorld([0,0,0],(time+o)-thisComp.frameDuration);\n\n\nv1 = length(vP1,vP2);\nreturn(v1);\n}\n\nfunction velLength (){\nfor(m = 0; m< f; m++){\nv+= timeVel(fD*-m,p);\n}\n\nreturn(v);\n}\n\nfunction findVelLength(){\nvL = velLength()/f;\n\nreturn(vL);\n}\n\n\n//create path\ncreatePath(posPoints,[],[],false)';
        return conCode;
      }
      function extraCode(inputLayer) {
        var extCode =
          '//OC Trails - leave this code be! \n \n function timeVel(e){return vP1=x.toWorld([0,0,0],time+e+thisComp.frameDuration),vP2=x.toWorld([0,0,0],time+e-thisComp.frameDuration),v1=length(vP1,vP2),v1}function velLength(){for(i=0;i<f;i++)v+=timeVel(fD*-i);return v}x=effect("Point ' +
          inputLayer +
          ' Control Layer")(1),velLenCheck=effect("OC Trails")("Velocity Based Line Length"),v=0,f=30,fD=thisComp.frameDuration,1==velLenCheck?velFinal=velLength()/f:velFinal=100,[velFinal,velFinal];';
        return extCode;
      }
      function applyTrailsPseudo() {
        ...
        var myPseudoEffect = {
          matchName: null,
          presetBinary: __BLOB__BLOB_000549__,
          presetName: "OC Trails.ffx",
        };
      ...
      var trailsButtonIcon = __BLOB__BLOB_000550__;
      ...
      var mathsGroupName = "LEAVE ME ALONE";
      var mathPathName = "Base path";
      var appGroupName = "Appearance";
      var appPathGroupName = "Appearance Paths";
      var appLayerName = "Appearance Path";
      function createPoint(shiftClicked) {
        curComp = app.project.activeItem;
        if (curComp == null) {
          alert(
            "Whoops! No Comp currently active. Either: \n \n -Select a Trails layer to add a control Point to \n \n -Select a Trails layer first and then a secondary layer to parent the control Point to",
          );
          return;
        }
        if (shiftClicked > 0) {
          curComp.layer(shiftClicked).selected = true;
        }
        var laySelected = curComp.selectedLayers;
        var l = 1;
        if (
          laySelected.length <= 2 &&
          laySelected.length > 0 &&
          laySelected[0].comment.indexOf("OC TRAILS CONTROL") >= 0
        ) {
          var thisController = laySelected[0].comment;
          var controlNum =
          ...
          var l = Math.max.apply(null, pointArray) + 1;
          var isTrial = ocTa.t();
          var trailPointLimit = 2;
          if (isTrial && l > trailPointLimit) {
            alert(
              "The trial version is limited to " +
                trailPointLimit +
                " points per Trail layer. Sorry!",
            );
            return;
          }
          var trailLayer = curComp.layers.addNull();
          trailLayer.source.name = laySelected[0].name + "- Point " + l;
          trailLayer.comment = controlNum + " POINT " + l;
          var addLaySelect = laySelected[0]
            .property("ADBE Effect Parade")
            .addProperty("ADBE Layer Control");
          addLaySelect.name = "Point " + l + " Control Layer";
          if (laySelected.length == 2) {
            addLaySelect.property(1).setValue(trailLayer.index);
            trailLayer.parent = laySelected[1];
            if (laySelected[1].threeDLayer == true) {
              trailLayer.threeDLayer = true;
            }
            var trailParentAP = laySelected[1]
              .property("ADBE Transform Group")
              .property("ADBE Anchor Point").value;
            trailLayer
              .property("ADBE Transform Group")
              .property("ADBE Position")
              .setValue(trailParentAP);
          } else {
            addLaySelect.property(1).setValue(trailLayer.index);
          }
          addNewShapePaths(laySelected[0], l);
          addLaySelect.moveTo(l);
          trailLayer.moveAfter(laySelected[0]);
          trailLayer.selected = false;
          laySelected[0].selected = true;
        } else {
          alert(
            "Whoops! Either: \n \n -Select a Trails layer to add a control Point to \n \n -Select a Trails layer first and then a secondary layer to parent the control Point to",
          );
