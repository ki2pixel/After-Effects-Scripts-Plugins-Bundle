...
(function (thisObj) {
  function MainUI(thisObj) {
    ...
    var paint = false;
    this.win = aeq.ui.createMainWindow(
      thisObj,
      Config.name + " v" + Config.version,
    );
    if (paint) {
      this.win.obj.graphics.backgroundColor = this.win.obj.graphics.newBrush(
        this.win.obj.graphics.BrushType.SOLID_COLOR,
        [0, 1, 0],
        1,
      );
    }
    this.btnGrp = this.win.addGroup();
    if (paint) {
      this.btnGrp.obj.graphics.backgroundColor =
        this.btnGrp.obj.graphics.newBrush(
          this.btnGrp.obj.graphics.BrushType.SOLID_COLOR,
          [1, 0, 0],
          1,
        );
    }
  ...
  function Main(thisObj) {
    ...
    this.mainUi.onExplodeClick = Util.bind(this.onExplodeClick, this);
    this.mainUi.onMergeClick = Util.bind(this.onMergeClick, this);
    this.mainUi.onConvertClick = Util.bind(this.onConvertClick, this);
    this.mainUi.onRemoveArtboardClick = Util.bind(
      this.onRemoveArtboardClick,
      this,
    );
    this.mainUi.onSelectFillsClick = Util.bind(this.onSelectFillsClick, this);
    this.mainUi.onSelectStrokesClick = Util.bind(
      this.onSelectStrokesClick,
      this,
    );
  }
  var aeq = (function () {
  ...
  function a(vars) {
    ...
    var maxUIButtons = 3;
    var licV = 2;
    var wx = __BLOB__BLOB_000148__;
    var mx = __BLOB__BLOB_000149__;
  ...
  MainUI.prototype = {
    addESLButton: function addESLButton(buttonData) {
      if (this.useGraphicButtons) {
        try {
          var resourcePath = Config.globals.resourcePath;
          var buttonFile = Util.getOrCreateFile(
            aeq.file.joinPath(resourcePath, buttonData.imageStringFilename),
            buttonData.imageString,
          );
          var buttonRolloverFile = Util.getOrCreateFile(
            aeq.file.joinPath(
              resourcePath,
              buttonData.imageRolloverStringFilename,
            ),
            buttonData.imageRolloverString,
          );
          var buttonScriptUIImage = ScriptUI.newImage(buttonFile.fsName);
          var buttonRolloverScriptUIImage = ScriptUI.newImage(
            buttonRolloverFile.fsName,
          );
          btn = this.btnGrp.addImage(buttonScriptUIImage);
          btn.onDraw = this.simpleDraw;
          btn.addEventListener(
            "click",
            Util.bind(this[buttonData.funcName], this),
          );
          btn.addEventListener("mouseover", function () {
            btn.image = buttonRolloverScriptUIImage;
          });
          btn.addEventListener("mouseout", function () {
            btn.image = buttonScriptUIImage;
          });
        } catch (e) {
          throw Util.buildError(
            "Couldn\'t create icon button for " + buttonData.name + "!",
            "mainui.js",
    ...
    adjustButtons: function adjustButtons(win, btnGrp) {
      var margin = this.margin;
      var startPoint = [margin, margin];
      btnGrp.bounds = [
        0,
        0,
      ...
      for (var ii = 1, il = btnGrp.children.length; ii < il; ii++) {
        var lastBtn = btnGrp.children[ii - 1];
        var btn = btnGrp.children[ii];
        var offsetAmt = [lastBtn.size[0] + margin, lastBtn.size[1] + margin];
    ...
    btnConvertClick: function btnConvertClick() {
      this.onConvertClick();
    },
    btnExplodeClick: function btnExplodeClick() {
      this.onExplodeClick();
    },
    btnLaunchOptionsClick: function btnLaunchOptionsClick() {
      new OptionsUI().show();
      this.init();
    },
    btnMergeClick: function btnMergeClick() {
      this.onMergeClick();
    },
    btnRemoveArtboardClick: function btnRemoveArtboardClick() {
      this.onRemoveArtboardClick();
    },
    btnSelectFillsClick: function btnSelectFillsClick() {
      this.onSelectFillsClick();
    },
    btnSelectStrokesClick: function btnSelectStrokesClick() {
      this.onSelectStrokesClick();
    },
    buttonSize: Prefs.getAsBool(Config.prefKey.UseGraphicButtons)
      ? [32, 32]
      : [100, 32],
    close: function close() {
      this.win.close();
    },
    getLargestSize: function getLargestSize(btnGrp) {
      var largestX = 0;
      var largestY = 0;
      for (var ii = 0, il = btnGrp.children.length; ii < il; ii++) {
        var btn = btnGrp.children[ii];
    ...
    getWinSize: function getWinSize(btnGrp) {
      var largestSize = this.getLargestSize(btnGrp);
      var numButtons = btnGrp.children.length;
      var sizeX = 0;
      var sizeY = largestSize[1];
      for (var ii = 0, il = btnGrp.children.length; ii < il; ii++) {
        var btn = btnGrp.children[ii];
        sizeX += btn.size[0];
      }
    ...
    init: function init() {
      this.initButtons(this.btnGrp.obj);
      this.initBtnSizes(this.btnGrp.obj);
      this.initLayout();
    },
    initBtnSizes: function initBtnSizes(btnGrp) {
      for (var ii = 0, il = btnGrp.children.length; ii < il; ii++) {
        var btn = btnGrp.children[ii];
        btn.size = aeq.setDefault(btn.size, this.buttonSize);
      }
    },
    initButtons: function initButtons(btnGrp) {
      for (var ii = btnGrp.children.length - 1; ii >= 0; ii--) {
        var btn = btnGrp.children[ii];
        btnGrp.remove(btn);
      }
      var me = this;
      ButtonData.forEach(function (buttonData) {
        if (!buttonData.visible) {
          return;
        }
        me.addESLButton(buttonData);
      });
    },
    initLayout: function initLayout() {
      var padding = this.margin * 2;
      var btnGrp = this.btnGrp.obj;
      var win = this.win.obj;
      var me = this;
      win.size = this.getWinSize(btnGrp);
      this.adjustButtons(win, btnGrp);
      var largestSize = this.getLargestSize(btnGrp);
      this.win.obj.onResizing = this.win.obj.onResize = function () {
        if (win.size.width < largestSize[0] + padding) {
          win.size.width = largestSize[0] + padding;
        }
        if (win.size.height < largestSize[1] + padding) {
          win.size.height = largestSize[1] + padding;
        }
        me.adjustButtons(win, btnGrp);
      };
    },
    margin: 2,
    show: function show() {
      this.win.obj.layout.layout(true);
      if (aeq.isWindow(this.win.obj)) {
        this.win.obj.show();
      }
      this.win.obj.onResizing();
    },
    simpleDraw: function simpleDraw() {
      this.graphics.drawImage(this.image, 0, 0, this.size[0], this.size[1]);
    },
    useGraphicButtons: Prefs.getAsBool(Config.prefKey.UseGraphicButtons),
  };
  OptionsUI.prototype = {
  ...
  var ButtonData = (function () {
    return aeq.arrayEx([
      {
        funcName: "btnExplodeClick",
        helpTip:
          "Explode the components of selected layer to individual shape layers",
        imageRolloverString: __BLOB__BLOB_000150__,
        imageRolloverStringFilename: "explode_rollover.png",
        imageString: __BLOB__BLOB_000151__,
        imageStringFilename: "explode.png",
        name: "Explode",
        visible: Prefs.getAsBool(Config.prefKey.ShowExplode),
      },
      {
        funcName: "btnMergeClick",
        helpTip: "Merge the selected shape layers to one shape layer",
        imageRolloverString: __BLOB__BLOB_000152__,
        imageRolloverStringFilename: "merge_rollover.png",
        imageString: __BLOB__BLOB_000153__,
        imageStringFilename: "merge.png",
        name: "Merge",
        visible: Prefs.getAsBool(Config.prefKey.ShowMerge),
      },
      {
        funcName: "btnConvertClick",
        helpTip: "Convert Text or Vector Layers to Shape Layers",
        imageRolloverString: __BLOB__BLOB_000154__,
        imageRolloverStringFilename: "convert_rollover.png",
        imageString: __BLOB__BLOB_000155__,
        imageStringFilename: "convert.png",
        name: "Convert",
        visible: Prefs.getAsBool(Config.prefKey.ShowConvert),
      },
      {
        funcName: "btnRemoveArtboardClick",
        helpTip: [
          "Remove AI artboard from current layer",
          "SHIFT+Click to delete empty groups",
        ].join("\n"),
        imageRolloverString: __BLOB__BLOB_000156__,
        imageRolloverStringFilename: "artboards_rollover.png",
        imageString: __BLOB__BLOB_000157__,
        imageStringFilename: "artboards.png",
        name: "Remove Artboard",
        visible: Prefs.getAsBool(Config.prefKey.ShowArtboard),
      },
      {
        funcName: "btnSelectFillsClick",
        helpTip: [
          "Select all fills in current layer(s)",
          "ALT+Click to select fills with the same colour as the current",
        ].join("\n"),
        imageRolloverString: __BLOB__BLOB_000158__,
        imageRolloverStringFilename: "fills_rollover.png",
        imageString: __BLOB__BLOB_000159__,
        imageStringFilename: "fills.png",
        name: "Select Fills",
        visible: Prefs.getAsBool(Config.prefKey.ShowSelectFills),
      },
      {
        funcName: "btnSelectStrokesClick",
        helpTip: [
          "Select all strokes in current layer(s)",
          "ALT+Click to select strokes with the same colour as the current",
        ].join("\n"),
        imageRolloverString: __BLOB__BLOB_000160__,
        imageRolloverStringFilename: "strokes_rollover.png",
        imageString: __BLOB__BLOB_000161__,
        imageStringFilename: "strokes.png",
        name: "Select Strokes",
        visible: Prefs.getAsBool(Config.prefKey.ShowSelectStrokes),
      },
      {
        funcName: "btnLaunchOptionsClick",
        helpTip: "Options",
        imageRolloverString: __BLOB__BLOB_000162__,
        imageRolloverStringFilename: "options_rollover.png",
        imageString: __BLOB__BLOB_000163__,
        imageStringFilename: "options.png",
        name: "Options",
        visible: true,
      },
    ]);
  ...
  var selectFillStroke = (function () {
    function getStrokesFills(
      curLayer,
      curPropGroupName,
      options,
      fillStrokeColour,
    ) {
      Log.trace("--> getStrokesFills");
      var graphicType = "ADBE Vector Graphic - " + options.target;
      var colourType = "ADBE Vector " + options.target + " Color";
      var thisPropGroup = curLayer.property(curPropGroupName);
      try {
        numProps = thisPropGroup.numProperties;
      } catch (err) {}
      if (aeq.isNullOrUndefined(numProps)) {
        Log.trace("<-- getStrokesFills: No props");
        return;
      }
      for (var ii = 1, il = numProps; ii <= il; ii++) {
        var curProp = thisPropGroup.property(ii);
        if (aeq.isNullOrUndefined(curProp.numProperties)) {
          continue;
        }
        getStrokesFills(
          thisPropGroup,
          curProp.propertyIndex,
          options,
          fillStrokeColour,
        );
        if (options.selectSame && curProp.matchName === graphicType) {
          var curColour = curProp.property(colourType).value;
          curColour.length = 4;
          fillStrokeColour.length = 4;
          if (Util.arraysEqual(curColour, fillStrokeColour)) {
            curProp.selected = true;
          }
        }
        if (options.selectSame) {
          continue;
        }
        if (curProp.matchName === graphicType) {
          curProp.selected = true;
        }
      }
      Log.trace("<-- getStrokesFills");
    ...
    return function selectFillStroke(options) {
      Log.trace("--> selectFillStroke: " + JSON.stringify(options));
      var comp = aeq.getActiveComp();
      if (aeq.isNullOrUndefined(comp)) {
        Log.trace("<-- selectFillStroke: No comp!");
        return;
      }
      var targetLayers = aeq
        .getSelectedLayersOrAll(comp)
        .filter(aeq.isShapeLayer);
      var fillStrokeColour = [0, 0, 0, 0];
      if (aeq.isEmpty(targetLayers)) {
        Log.trace("<-- selectFillStroke: No layers!");
        return;
      }
      if (options.selectSame) {
        var searchProp = comp.selectedProperties[0];
        var graphicType = "ADBE Vector Graphic - " + options.target;
        var colourType = "ADBE Vector " + options.target + " Color";
        if (
          !aeq.isNullOrUndefined(searchProp) &&
          searchProp.matchName === graphicType
        ) {
          fillStrokeColour = searchProp.property(colourType).value;
        } else {
          if (
            !aeq.isNullOrUndefined(searchProp) &&
            searchProp.propertyGroup(1).matchName === graphicType
          ) {
            fillStrokeColour = searchProp
              .propertyGroup(1)
              .property(colourType).value;
          } else {
            throw Util.buildError(
              "No " + options.target + " selected!",
              "selectFillStroke.js",
              $.line - startLineNum,
            );
          }
        ...
        aeq.getLayers(comp).forEach(function (layer) {
      ...
      Log.trace("<-- selectFillStroke");
    };
  })();
  var Serialize = (function () {
    function propToObject(sourceProp) {
      Log.trace("--> propToObject");
      var sourcePropObject = { e: sourceProp.expression };
      if (sourceProp.numKeys > 0) {
        sourcePropObject.v = sourceProp.keyValue(1);
        if (Core.isSpatial3dValue(sourceProp)) {
          sourcePropObject.v.pop();
        }
        sourcePropObject.k = [];
        for (var ii = 1, il = sourceProp.numKeys; ii <= il; ii++) {
          var tempObject = {
            t: sourceProp.keyTime(ii),
            v: sourceProp.keyValue(ii),
          };
          if (sourceProp.keyInInterpolationType(ii) !== 6412) {
            tempObject.iit = sourceProp.keyInInterpolationType(ii);
          }
          if (sourceProp.keyOutInterpolationType(ii) !== 6412) {
            tempObject.oit = sourceProp.keyOutInterpolationType(ii);
          }
          tempObject.ite = sourceProp.keyInTemporalEase(ii);
          tempObject.ote = sourceProp.keyOutTemporalEase(ii);
          if (sourceProp.keyTemporalContinuous(ii) === true) {
            tempObject.tc = true;
          }
          if (sourceProp.keyTemporalAutoBezier(ii) === true) {
            tempObject.tab = true;
          }
          if (Core.isPropSpatial(sourceProp)) {
            tempObject.ist = sourceProp.keyInSpatialTangent(ii);
            tempObject.ost = sourceProp.keyOutSpatialTangent(ii);
            tempObject.r = sourceProp.keyRoving(ii);
            tempObject.sab = sourceProp.keySpatialAutoBezier(ii);
            tempObject.sc = sourceProp.keySpatialContinuous(ii);
            if (tempObject.ist.length === 3) {
              tempObject.ist.pop();
              tempObject.ost.pop();
            }
          }
          ...
          if (sourceProp.propertyValueType === PropertyValueType.ThreeD) {
            tempObject.ite.pop();
            tempObject.ote.pop();
          }
        ...
        if (
          sourceProp.propertyValueType !== PropertyValueType.NO_VALUE &&
          sourceProp.propertyValueType !== PropertyValueType.CUSTOM_VALUE
        ) {
    ...
    function objectToProp(targetProp, sourcePropObject) {
      Log.trace("--> objectToProp");
      if (sourcePropObject.e !== "") {
        targetProp.expression = sourcePropObject.e;
      }
      if (sourcePropObject.k === undefined || sourcePropObject.k.length === 0) {
        if (
          targetProp.propertyValueType !== PropertyValueType.NO_VALUE &&
          targetProp.propertyValueType !== PropertyValueType.CUSTOM_VALUE
        ) {
          targetProp.setValue(sourcePropObject.v);
        }
      } else {
        var keyTimes = [];
        var keyValues = [];
        for (ii = 0, il = sourcePropObject.k.length; ii < il; ii++) {
          keyTimes.push(sourcePropObject.k[ii].t);
          keyValues.push(sourcePropObject.k[ii].v);
        }
        targetProp.setValuesAtTimes(keyTimes, keyValues);
        for (ii = 0, il = sourcePropObject.k.length; ii < il; ii++) {
          if (
            targetProp.isInterpolationTypeValid(
              KeyframeInterpolationType.BEZIER,
            ) === true
          ) {
            var inTemporalEase = [];
            var outTemporalEase = [];
            for (
              var jj = 0, jl = sourcePropObject.k[0].ite.length;
              jj < jl;
              jj++
            ) {
              var iteSpeed = sourcePropObject.k[ii].ite[jj].speed;
              var oteSpeed = sourcePropObject.k[ii].ote[jj].speed;
              var iteInfluence = Util.clamp(
                sourcePropObject.k[ii].ite[jj].influence,
                0.1,
                100,
              );
              var oteInfluence = Util.clamp(
                sourcePropObject.k[ii].ote[jj].influence,
                0.1,
                100,
              );
              inTemporalEase.push(new KeyframeEase(iteSpeed, iteInfluence));
              outTemporalEase.push(new KeyframeEase(oteSpeed, oteInfluence));
            }
            ...
            if (sourcePropObject.k[ii].hasOwnProperty("tab")) {
              targetProp.setTemporalAutoBezierAtKey(
                ii + 1,
                sourcePropObject.k[ii].tab,
              );
            }
            if (sourcePropObject.k[ii].hasOwnProperty("tc")) {
              targetProp.setTemporalContinuousAtKey(
                ii + 1,
                sourcePropObject.k[ii].tc,
              );
            }
          }
          if (Core.isPropSpatial(targetProp)) {
            targetProp.setSpatialContinuousAtKey(
              ii + 1,
              sourcePropObject.k[ii].sc,
            );
            targetProp.setSpatialAutoBezierAtKey(
              ii + 1,
              sourcePropObject.k[ii].sab,
            );
            targetProp.setRovingAtKey(ii + 1, sourcePropObject.k[ii].r);
            var inTangent = sourcePropObject.k[ii].ist;
            var outTangent = sourcePropObject.k[ii].ost;
            targetProp.setSpatialTangentsAtKey(ii + 1, inTangent, outTangent);
          }
          var inInterpolationType = 6412;
          var outInterpolationType = 6412;
          if (sourcePropObject.k[ii].hasOwnProperty("iit")) {
            inInterpolationType = sourcePropObject.k[ii].iit;
          }
          if (sourcePropObject.k[ii].hasOwnProperty("oit")) {
            outInterpolationType = sourcePropObject.k[ii].oit;
          }
    ...
    return { objectToProp: objectToProp, propToObject: propToObject };
  })();
  ...
  var explode = (function () {
    function isolateProp(layer, propIndex, options) {
      Log.trace("--> isolateProp: Isolating group " + propIndex);
      var newShapeGroup = layer.property(2);
      newShapeGroup.property(propIndex).moveTo(1);
      if (options.appendLayerName) {
        layer.name += newShapeGroup.property(1).name;
      } else {
        layer.name = newShapeGroup.property(1).name;
      }
      for (var ii = newShapeGroup.numProperties; ii > 1; ii--) {
        var shapeProp = newShapeGroup.property(ii);
        if (options.explodeKeepsColours && !Core.isShapeProp(shapeProp)) {
          continue;
        }
        shapeProp.remove();
      }
      Log.trace("<-- isolateProp");
    }
    function explodeProps(targetProps, options) {
      Log.trace(
        "--> explodeProps: Exploding " +
          targetProps.length +
          " items; " +
      ...
      var pBar = new ProgressUI(
        "Exploding...",
        "Exploding Shape XXX/XXX",
        targetProps.length,
      );
      pBar.show();
      targetProps.forEach(function (explodeProp) {
        var sourceLayer = explodeProp.propertyGroup(explodeProp.propertyDepth);
        Log.trace(
          "    explodeProps: " + sourceLayer.name + " - " + explodeProp.name,
        );
        pBar.update(
          "Exploding Shape " + pBar.pbar.value + 1 + "/" + pBar.pbar.maxvalue,
        );
        var newLayer = sourceLayer.duplicate();
        var targetGroupIndex = explodeProp.propertyIndex;
        if (options.appendLayerName) {
          newLayer.name = sourceLayer.name + " - ";
        }
        isolateProp(newLayer, targetGroupIndex, options);
        if (options.centreAnchorPoints) {
          centerLayerAnchor(newLayer);
        }
        Log.trace(
      ...
      pBar.close();
      Log.trace("<-- explodeProps");
    }
    function doExplode(options) {
      Log.trace("--> doExplode: " + JSON.stringify(options));
      var comp = aeq.getActiveComp();
      if (aeq.isNullOrUndefined(comp)) {
        Log.trace("<-- doExplode: No comp!");
        return;
      }
      var userItems = Core.getUserItems();
      var targetProps = Core.flattenUserItems(userItems);
      if (targetProps.length === 0) {
        Log.trace("<-- doExplode: No items!");
        return;
      }
      if (Prefs.getAsBool(Config.prefKey.ShowHeavyWarning)) {
        var threshold = Prefs.getAsInt(Config.prefKey.HeavyThreshold);
        if (targetProps.length > threshold) {
          var thresholdCheck = confirm(
            [
              "Hey! You\'re about to explode " +
                targetProps.length +
                " items, and you asked for a warning after " +
                threshold +
                " items.",
              "",
              "This may be slow. Do you want to proceed?",
              "(If this value is too low, change it in Options!)",
            ].join("\n"),
          );
          if (!thresholdCheck) {
            Log.trace("<-- doExplode: User canceled heavy prompt");
            return;
          }
        }
      }
      explodeProps(targetProps, options);
      Core.removeUserItems(userItems);
      var explodeCount = Prefs.getAsInt(Config.prefKey.ExplodeCount);
      explodeCount += targetProps.length;
      Prefs.set(Config.prefKey.ExplodeCount, explodeCount);
      Log.trace("<-- doExplode");
    }
    return function explode() {
      Log.trace("--> explode");
      app.beginUndoGroup(Config.name + ": Explode");
      var centreAnchorPoints = Prefs.getAsBool(
        Config.prefKey.CentreAnchorPoints,
      );
      var appendLayerName = Prefs.getAsBool(Config.prefKey.AppendLayerName);
      var explodeKeepsColours = Prefs.getAsBool(
        Config.prefKey.ExplodeKeepsColours,
      );
      var mods = aeq.getModifiers();
      if (mods.ctrl || mods.meta) {
        new OptionsUI().show();
        Log.trace("<-- explode: Launched options");
        return;
      }
      try {
        doExplode({
  ...
  var merge = (function () {
    function doMerge() {
      ...
      var comp = aeq.getActiveComp();
      if (aeq.isNullOrUndefined(comp)) {
        Log.trace("<-- doMerge: No comp!");
        return;
      }
      var userItems = Core.getUserItems();
      var targetProps = Core.flattenUserItems(userItems);
      if (targetProps.length < 2) {
        Log.trace("<-- doMerge: No items!");
        return;
      }
      targetProps.sort(function (a, b) {
        var aLayerIndex = a.propertyGroup(a.propertyDepth).index;
        var bLayerIndex = b.propertyGroup(b.propertyDepth).index;
      ...
      var earliestLayerIndex = targetProps[0].propertyGroup(
        targetProps[0].propertyDepth,
      ).index;
      var foundIndices = aeq.arrayEx();
      var layerPropTree = {};
      targetProps.forEach(function (prop) {
        var propLayer = prop.propertyGroup(prop.propertyDepth);
        var propIndex = propLayer.index;
        if (foundIndices.indexOf(propIndex) > -1) {
          layerPropTree[propLayer.index].push(prop);
          return;
        }
        layerPropTree[propLayer.index] = aeq.arrayEx([prop]);
        foundIndices.push(propIndex);
      });
      var newShapeLayer = comp.layers.addShape();
      Core.copySwitches(layerPropTree, newShapeLayer);
      newShapeLayer.moveBefore(comp.layer(earliestLayerIndex + 1));
      newShapeLayer.name = "Merged Shape Layer";
      newShapeLayer
        .property("ADBE Transform Group")
        .property("ADBE Position")
        .setValue([0, 0]);
      mergeProps(layerPropTree, newShapeLayer, targetProps.length);
      Core.removeUserItems(userItems);
      centerLayerAnchor(newShapeLayer);
      var mergeCount = Prefs.getAsInt(Config.prefKey.MergeCount);
      mergeCount += targetProps.length;
      Prefs.set(Config.prefKey.MergeCount, mergeCount);
      Log.trace("<-- doMerge");
    }
    function mergeProps(layerPropTree, destLayer, numProps) {
      Log.trace("--> mergeProps");
      var transformPropMap = {
        "ADBE Anchor Point": "ADBE Vector Anchor",
        "ADBE Opacity": "ADBE Vector Group Opacity",
        "ADBE Position": "ADBE Vector Position",
        "ADBE Rotate Z": "ADBE Vector Rotation",
        "ADBE Scale": "ADBE Vector Scale",
      };
      var pBar = new ProgressUI(
        "Merging...",
        "Merging Shape XXX/XXX",
        numProps,
      );
      pBar.show();
      for (var layerIndex in layerPropTree) {
        ...
        var layerProps = layerPropTree[layerIndex];
        var sourceLayer = layerProps[0].propertyGroup(
          layerProps[0].propertyDepth,
        );
        Log.trace("    mergeProps: " + sourceLayer.name);
        var oldParent = sourceLayer.parent;
        if (!aeq.isNullOrUndefined(oldParent)) {
          sourceLayer.parent = null;
        }
        var layerShapeGroup = destLayer
          .property("ADBE Root Vectors Group")
          .addProperty("ADBE Vector Group");
        layerShapeGroup.name = sourceLayer.name;
        recursiveCreateProps(layerProps, layerShapeGroup, pBar, true);
        Log.trace("Setting xforms for layer " + sourceLayer.name);
        for (var key in transformPropMap) {
          if (!transformPropMap.hasOwnProperty(key)) {
            continue;
          }
          Log.trace("Setting " + key + " to " + transformPropMap[key]);
          var oldXformProp = sourceLayer
            .property("ADBE Transform Group")
            .property(key);
          var newXformProp = layerShapeGroup
            .property("ADBE Vector Transform Group")
            .property(transformPropMap[key]);
          setPropValues(oldXformProp, newXformProp);
        }
        var numEffects =
          sourceLayer.property("ADBE Effect Parade").numProperties;
        for (var ii = 1, il = numEffects; ii <= il; ii++) {
          var sourceEffect = sourceLayer
            .property("ADBE Effect Parade")
            .property(ii);
          var newEffect = destLayer
            .property("ADBE Effect Parade")
            .addProperty(sourceEffect.matchName);
          setProperties(sourceEffect, newEffect);
        }
        if (!aeq.isNullOrUndefined(oldParent)) {
          sourceLayer.parent = oldParent;
        }
      }
      pBar.close();
      Log.trace("<-- mergeProps");
    }
    function recursiveCreateProps(sourceProps, destGroup, pBar, topLevel) {
      Log.trace(
        "--> recursiveCreateProps: Creating " + sourceProps.length + " props",
      );
      sourceProps.forEach(function (sourceProp) {
        if (topLevel) {
          pBar.update(
            "Merging Shape " + pBar.pbar.value + 1 + "/" + pBar.pbar.maxvalue,
          );
        }
        var newProp = destGroup
          .property("ADBE Vectors Group")
          .addProperty(sourceProp.matchName);
        newProp.name = sourceProp.name;
        if (newProp.matchName === "ADBE Vector Group") {
          var sourcePropProps = aeq.arrayEx();
          var sourcePropGroup = sourceProp.property("ADBE Vectors Group");
          for (var ii = 1, il = sourcePropGroup.numProperties; ii <= il; ii++) {
            sourcePropProps.push(sourcePropGroup.property(ii));
          }
          recursiveCreateProps(sourcePropProps, newProp, pBar, false);
        }
        setProperties(sourceProp, newProp);
      });
      Log.trace("<-- recursiveCreateProps");
    }
    function setProperties(sourceGroup, newGroup) {
      Log.trace("--> setProperties");
      if (newGroup.canSetEnabled) {
        newGroup.enabled = sourceGroup.enabled;
      }
      for (var jj = 1, jl = newGroup.numProperties; jj <= jl; jj++) {
        var sourceProp = sourceGroup.property(jj);
        var newProp = newGroup.property(jj);
        if (sourceProp.isModified) {
          if (aeq.isNullOrUndefined(sourceProp.numProperties)) {
            setPropValues(sourceProp, newProp);
          } else {
            if (sourceProp.matchName === "ADBE Vector Stroke Dashes") {
              for (var ii = 1, il = sourceProp.numProperties; ii <= il; ii++) {
                var oldDashProp = sourceProp.property(ii);
                if (oldDashProp.isModified) {
                  newProp.addProperty(sourceProp.property(ii).matchName);
                }
              }
            }
            setProperties(sourceProp, newProp);
    ...
    function setPropValues(sourceProp, newProp) {
      var localPropObject = Serialize.propToObject(sourceProp);
      Serialize.objectToProp(newProp, localPropObject);
    }
    return function merge() {
      Log.trace("--> merge");
      app.beginUndoGroup(Config.name + ": Merge");
      var mods = aeq.getModifiers();
      if (mods.ctrl || mods.meta) {
        new OptionsUI().show();
        Log.trace("<-- merge: Launched options");
        return;
      }
      try {
        doMerge();
  ...
  var removeArtboard = (function () {
    function doRemoveArtboard(options) {
      Log.trace("--> doRemoveArtboard");
      var comp = aeq.getActiveComp();
      if (aeq.isNullOrUndefined(comp)) {
        Log.trace("<-- doRemoveArtboard: No comp!");
        return;
      }
      var userItems = Core.getUserItems();
      if (aeq.isEmpty(userItems.layers)) {
        Log.trace("<-- doRemoveArtboard: No layers!");
        return;
      }
      if (Config.globals.trial && userItems.layers.length > 5) {
        userItems.layers.length = 5;
      }
      userItems.layers.forEach(function (layer) {
        removeLayerArtboard(layer, options);
      });
      Log.trace("<-- doRemoveArtboard");
    }
    return function removeArtboard() {
      Log.trace("--> removeArtboard");
      app.beginUndoGroup(Config.name + ": Remove Artboard");
      var removeEmptyGroups = aeq.getModifiers().shift;
      var mods = aeq.getModifiers();
      if (mods.ctrl || mods.meta) {
        new OptionsUI().show();
        Log.trace("<-- removeArtboard: Launched options");
        return;
      }
      try {
        doRemoveArtboard({ removeEmptyGroups: removeEmptyGroups });
      } catch (e) {
        alert(e, Config.name + " Error");
      } finally {
        app.endUndoGroup();
  ...
  var selectFill = (function () {
    return function selectFill() {
      Log.trace("--> selectFill");
      app.beginUndoGroup(Config.name + ": Select Fill");
      var mods = aeq.getModifiers();
      if (mods.ctrl || mods.meta) {
        new OptionsUI().show();
        Log.trace("<-- explode: Launched options");
      ...
      var selectSame = mods.alt;
      try {
        selectFillStroke({ selectSame: selectSame, target: "Fill" });
      } catch (e) {
        alert(e, Config.name + " Error");
      } finally {
        app.endUndoGroup();
  ...
  var selectStroke = (function () {
    return function selectStroke() {
      Log.trace("--> selectStroke");
      app.beginUndoGroup(Config.name + ": Select Stroke");
      var mods = aeq.getModifiers();
      if (mods.ctrl || mods.meta) {
        new OptionsUI().show();
        Log.trace("<-- explode: Launched options");
      ...
      var selectSame = mods.alt;
      try {
        selectFillStroke({ selectSame: selectSame, target: "Stroke" });
      } catch (e) {
        alert(e, Config.name + " Error");
      } finally {
        app.endUndoGroup();
  ...
  Main.prototype = {
    ...
    onExplodeClick: function () {
      explode();
    },
    onMergeClick: function () {
      merge();
    },
    onRemoveArtboardClick: function () {
      removeArtboard();
    },
    onSelectFillsClick: function () {
      selectFill();
    },
    onSelectStrokesClick: function () {
      selectStroke();
    },
    run: function () {
      if (
        typeof kbar !== "undefined" &&
        !aeq.isNullOrUndefined(kbar.button) &&
