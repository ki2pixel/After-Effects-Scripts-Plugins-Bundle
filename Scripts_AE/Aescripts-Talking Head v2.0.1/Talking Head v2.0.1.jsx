...
function checkForControls(f) {
  try {
    var g = updateLayer(mst, ctrl_name);
    if (!g) {
      var i = createControls();
    }
  } catch (h) {
    alert(h);
  }
}
function updateLayer(h, j) {
  var i = false;
  for (var g = 1; g < h.layers.length + 1; g += 1) {
    if (h.layers[g].name == ctrl_name) {
      var f = h.layers[g];
      i = true;
    }
  }
...
function audio(c) {
  ctrl_layer("ADBE Effect Parade")(aud_n)(1).expression =
    exp_header +
    'var vo;\ntry\n{\nvo = thisComp.layer("' +
    aud_n +
    '").effect(1)(1);\n}\ncatch(err)\n{\nvo = 0;\n}\nvo;';
...
function createControls() {
  ctrl_layer = mst.layers.addNull();
  ctrl_layer.name = ctrl_name;
  ctrl_layer.source.name = ctrl_name;
  controlLayer = ctrl_layer.index;
  var x = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Slider Control");
  x.name = aud_n;
  audio(ctrl_layer);
  var t = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Slider Control");
  t.name = mot_n;
  var z = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Checkbox Control");
  z.name = lms_n;
  var E = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Slider Control");
  E.name = moc_n;
  E(1).expression =
    exp_header +
    'var count = 0;\nfor (i = 1; i <= thisComp.numLayers; i++)\n{\n\tt_layer = thisComp.layer(i);\n\tif (t_layer.name.match("' +
    mouth_n +
    spc +
    'Open"))\n\t{\n\t\tcount++;\n\t}\n}\ncount;    \n';
  var B = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Slider Control");
  B.name = msf_n;
  var H = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Slider Control");
  H.name = mo_n;
  H(1).setValue([1]);
  H(1).expression =
    exp_header +
    'sldr = effect("' +
    mo_n +
    '")(1);\nif (sldr.numKeys > 0)\n{\nvar ms;\nif (time < nearestKey(time).time && nearestKey(time).index >1)\n{\nms = Math.floor(key(nearestKey(time).index-1));\n}\nelse\n{\nms = Math.floor(nearestKey(time).value);\n}\n}\nelse\n{\nms = Math.floor(value);\n}\n if (ms < 1) { ms = 1; }\nms;';
  var D = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Slider Control");
  D.name = mc_n;
  D(1).setValue([1]);
  D(1).expression =
    exp_header +
    ...
    mc_n +
    '")(1);\nif (sldr.numKeys > 0)\n{\nvar ms;\nif (time < nearestKey(time).time && nearestKey(time).index >1)\n{\nms = Math.floor(key(nearestKey(time).index-1));\n}\nelse\n{\nms = Math.floor(nearestKey(time).value);\n}\n}\nelse\n{\nms = Math.floor(value);\n}\n if (ms < 1) { ms = 1; }\nms;';
  var s = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Slider Control");
  s.name = jsf_n;
  var m = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Point Control");
  m.name = ld_n;
  var y = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Point Control");
  y.name = ls_n;
  var i = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Slider Control");
  i.name = bl_int;
  var A = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Slider Control");
  A.name = bl_sp;
  var J = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Slider Control");
  J.name = sq_n;
  var C = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Slider Control");
  C.name = sqos_n;
  var L = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Slider Control");
  L.name = ebt_n;
  var K = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Slider Control");
  K.name = ebr_n;
  var F = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Slider Control");
  F.name = ebos_n;
  var G = ctrl_layer("ADBE Effect Parade").addProperty("ADBE Slider Control");
  G.name = ebp_n;
  ctrl_layer("ADBE Effect Parade")(mot_n)(1).setValue([1]);
  ctrl_layer("ADBE Effect Parade")(msf_n)(1).setValue([2]);
  ctrl_layer("ADBE Effect Parade")(jsf_n)(1).setValue([1]);
  ctrl_layer("ADBE Effect Parade")(bl_int)(1).setValue([100]);
  ctrl_layer("ADBE Effect Parade")(bl_sp)(1).setValue([90]);
}
function Feature(f, g, e) {
  this.dir_label;
  this.pos_m;
  this.rot_m;
  this.scale_m;
  ...
  this.setName = function (c) {
    if (c > 0) {
      this.f_index = spc + (c + 1).toString();
    } else {
      this.f_index = "";
    }
    this.f_name = g + spc + this.dir_label + this.f_index;
    return this.f_name;
  };
}
function rigBrow(g, h) {
  try {
    var k = new Feature(g, brow_n, h);
    if (g && g.hasVideo) {
      g("ADBE Transform Group")("ADBE Position").expression =
        exp_header +
        'Rbro = thisComp.layer("' +
        ctrl_name +
        '")("ADBE Effect Parade")("' +
        ebr_n +
        '")(1); \rObro = thisComp.layer("' +
        ctrl_name +
        '")("ADBE Effect Parade")("' +
        ebos_n +
        '")(1); \rPbro = thisComp.layer("' +
        ctrl_name +
        '")("ADBE Effect Parade")("' +
        ebp_n +
        '")(1); \r[value[0] ' +
        ...
        k.pos_m +
        " Obro]";
      g.property("ADBE Transform Group")("ADBE Rotate Z").expression =
        exp_header +
        'tilt=thisComp.layer("' +
        ctrl_name +
        '")("ADBE Effect Parade")("' +
        ...
        k.rot_m +
        "tilt;";
      var i = getFeatures(brow_n + spc + k.dir_label, g.index);
      g.name = k.setName(i.length);
    }
  } catch (j) {
    alert(j);
  }
}
function rigPupil(j, l) {
  var k = new Feature(j, pupil_n, l);
  if (j.selection != 0) {
    if (j.hasVideo) {
      j("ADBE Transform Group")("ADBE Position").expression =
        exp_header +
        'lookDir=thisComp.layer("' +
        ctrl_name +
        '")("ADBE Effect Parade")("' +
        ld_n +
        '")(1)-[50,50]; \rls = thisComp.layer("' +
        ctrl_name +
        '")("ADBE Effect Parade")("' +
        ls_n +
        '")(1)-[50,50]; \r[value[0]+(lookDir[0]' +
        k.pos_m +
        "ls[0]), value[1]+(lookDir[1]" +
        k.pos_m +
        "ls[1])];";
      var m = getFeatures(pupil_n + spc + k.dir_label, j.index);
      j.name = k.setName(m.length);
      var h = j.name.replace(pupil_n, eye_n);
      var i = getFeatures(h);
      if (i.length > 0 && j.trackMatteType == TrackMatteType.NO_TRACK_MATTE) {
        addPupilMask(j, i[0]);
      }
    }
  }
...
function rigEye(i, l) {
  var m = new Feature(i, eye_n, l);
  if (i.selection != 0) {
    if (i.hasVideo) {
      i("ADBE Transform Group")("ADBE Scale").expression =
        exp_header +
        'var sq=thisComp.layer("' +
        ctrl_name +
        '")("ADBE Effect Parade")("' +
        sq_n +
        '")(1); \rvar sqO=thisComp.layer("' +
        ctrl_name +
        '")("ADBE Effect Parade")("' +
        sqos_n +
        '")(1); \rvar e_opn = [value[0], Math.max(1, value[1]-sq' +
        m.scale_m +
        'sqO)];\rvar e_cls = [value[0], 1];\rvar interval = thisComp.layer("' +
        ctrl_name +
        '")("ADBE Effect Parade")("' +
        bl_int +
        '")(1)*0.01;\rvar bias = thisComp.layer("' +
        ctrl_name +
        '")("ADBE Effect Parade")("' +
        bl_sp +
        '")(1)*0.01;\rvar t = time % (interval*2);\rif (t > interval)\n{\nt = (interval*2) - t;\n}\nease(t, interval*bias, interval, e_opn, e_cls);\r';
      var j = getFeatures(eye_n + spc + m.dir_label, i.index);
      i.name = m.setName(j.length);
      var n = i.name.replace(eye_n, pupil_n);
      var k = getFeatures(n);
      if (k.length > 0) {
        b = k[0];
        if (b.trackMatteType == TrackMatteType.NO_TRACK_MATTE) {
          addPupilMask(b, i);
        }
      }
    }
...
function addPupilMask(i, k) {
  try {
    var h = i.name + spc + matte_n;
    var g = k.duplicate();
    removeExp(g);
    g.name = h;
    g.moveBefore(i);
    g.parent = k;
    g.trackMatteType = TrackMatteType.NO_TRACK_MATTE;
    i.trackMatteType = TrackMatteType.ALPHA;
  } catch (j) {
    alert(j);
  }
}
function rigJaw(c) {
  if (c.hasVideo) {
    c.name = jaw_n;
    c("ADBE Transform Group")("ADBE Scale").expression =
      exp_header +
      'sk=thisComp.layer("' +
      ctrl_name +
      '")("ADBE Effect Parade")("' +
      aud_n +
      '")(1); \rjs=thisComp.layer("' +
      ctrl_name +
      '")("ADBE Effect Parade")("' +
      jsf_n +
      '")(1); \rt=thisComp.layer("' +
      ...
      '")("ADBE Effect Parade")("' +
      mot_n +
      '")(1); \rif (sk>t) \r{ \r[value[0], value[1]+(sk*js)] \r}\r else\r { \r[value[0], value[1]] \r}';
  }
}
function attachToFace(f, d) {
...
function checkForFace(d) {
  var e = getFeatures(face_n, d.index);
  if (e.length > 0) {
    d.parent = e[0];
  }
}
function getFeatures(h) {
  var k = new Array();
  var f = app.project.activeItem;
  for (var g = 1; g <= f.layers.length; g += 1) {
    var j = f.layers[g];
    if (j.name.match(h) && !j.name.match(matte_n)) {
      k.push(j);
    }
  }
  return k;
}
function getFeatures(j, g) {
  var l = new Array();
  var m = app.project.activeItem;
  for (var h = 1; h <= m.layers.length; h += 1) {
    var k = m.layers[h];
    if (k.name.match(j) && !k.name.match(matte_n)) {
      if (h != g) {
        l.push(k);
      }
    }
...
function removeExp(h) {
  for (var g = 1; g <= h("ADBE Transform Group").numProperties; g += 1) {
    try {
      var j = h("ADBE Transform Group")(g);
      var f = j.expression;
      if (f) {
        if (f.match(exp_header)) {
          j.expression = "";
          j.expressionEnabled = false;
        }
      }
    } catch (i) {
      alert(i);
...
function init_mouth(c) {
  app.beginUndoGroup("Talking Head");
  mst = app.project.activeItem;
  if (mst) {
    var e = mst.selectedLayers;
    if (e) {
      checkForControls(mst);
      for (var d = 0; d < e.length; d += 1) {
        rigMouth(e[d], c ? "Closed" : "Open", c);
        checkForFace(e[d]);
      }
    } else {
      alert(
        "No Layer Selected\rPlease select a Layer to use as the " +
          mouth_n +
          spc +
          I,
      );
    ...
    alert("Please select a Composition in the Project window.");
  }
}
function rigMouth(m, h, o) {
  var i = ", ";
  var q = mouth_n + spc;
  var l = m;
  if (l.hasVideo) {
    if (!o) {
      var p = new Array();
      p = getFeatures(mouth_n + spc + "Open", l.index);
      l.name = (q + h + " " + p.length + 1).toString();
      l("ADBE Transform Group")("ADBE Opacity").expression =
        ...
        '")("ADBE Effect Parade")("' +
        mot_n +
        '")(1); \rmShape = thisComp.layer("' +
        ctrl_name +
        '")("ADBE Effect Parade")("' +
        mo_n +
        '")(1);  \rthisShape = parseInt(name.substring(name.lastIndexOf(" ")+1, name.length));\ncount = thisComp.layer("' +
        ctrl_name +
        '").effect("' +
        moc_n +
        '")(1);\nv_min = t * (thisShape);\nv_max = t * (thisShape+1);\nlinkShapeAudio = thisComp.layer("' +
        ctrl_name +
        '").effect("' +
        lms_n +
      ...
      l("ADBE Transform Group")("ADBE Scale").expression =
        ...
        '")("ADBE Effect Parade")("' +
        aud_n +
        '")(1); \rms=thisComp.layer("' +
        ctrl_name +
        '")("ADBE Effect Parade")("' +
        msf_n +
        '")(1); \r[value[0], value[1]+(sk*ms)]';
    }
    if (o) {
      var j = new Array();
      j = getFeatures(mouth_n + spc + "Closed", l.index);
      l.name = (q + h + " " + j.length + 1).toString();
      l("ADBE Transform Group")("ADBE Opacity").expression =
        exp_header +
        'aa=thisComp.layer("' +
        ctrl_name +
        ...
        '")("ADBE Effect Parade")("' +
        mc_n +
        '")(1);  \rthisShape = parseInt(name.substring(name.lastIndexOf(" ")+1, name.length));\n(aa<t && mShape == thisShape)?100:0;';
    }
  }
}
function rc_TalkingHeadMainFunc(g) {
  var f = g instanceof Panel ? g : new Window("panel", "Test Panel");
  f.orientation = "column";
  f.alignment = "right";
  var d = f.add("group");
  d.orientation = "row";
  d.alignment = "right";
  d.add("statictext", undefined, "Eyebrow");
  f.rigBrow_L = d.add("button", undefined, "Left").onClick = function () {
    app.beginUndoGroup("Talking Head");
    mst = app.project.activeItem;
    if (mst) {
      if (mst.selectedLayers.length > 0) {
        var k = mst.selectedLayers[0];
        checkForControls(mst);
        rigBrow(k, "L");
        checkForFace(k);
      } else {
  ...
  f.rigBrow_R = d.add("button", undefined, "Right").onClick = function () {
    app.beginUndoGroup("Talking Head");
    mst = app.project.activeItem;
    if (mst) {
      if (mst.selectedLayers.length > 0) {
  ...
  j.orientation = "row";
  j.alignment = "right";
  j.add("statictext", undefined, "Eye");
  f.rigEye_L = j.add("button", undefined, "Left").onClick = function () {
    app.beginUndoGroup("Talking Head");
    mst = app.project.activeItem;
    if (mst) {
      if (mst.selectedLayers.length > 0) {
  ...
  f.rigEye_R = j.add("button", undefined, "Right").onClick = function () {
    app.beginUndoGroup("Talking Head");
    mst = app.project.activeItem;
    if (mst) {
      if (mst.selectedLayers.length > 0) {
  ...
  i.orientation = "row";
  i.alignment = "right";
  i.add("statictext", undefined, "Pupil");
  f.rigPupil_L = i.add("button", undefined, "Left").onClick = function () {
    app.beginUndoGroup("Talking Head");
    mst = app.project.activeItem;
    if (mst) {
      if (mst.selectedLayers.length > 0) {
  ...
  f.rigPupil_R = i.add("button", undefined, "Right").onClick = function () {
    app.beginUndoGroup("Talking Head");
    mst = app.project.activeItem;
    if (mst) {
      if (mst.selectedLayers.length > 0) {
  ...
  h.orientation = "row";
  h.alignment = "right";
  h.add("statictext", undefined, "Mouth");
  f.rigMouth_open = h.add("button", undefined, "Open").onClick = function () {
    init_mouth(false);
  };
  f.righMouth_closed = h.add("button", undefined, "Closed").onClick =
    function () {
      init_mouth(true);
    };
  var e = f.add("group");
  e.orientation = "row";
  e.alignment = "right";
  e.add("statictext", undefined, "Head");
  f.rigJaw = e.add("button", undefined, "Jaw").onClick = function () {
    app.beginUndoGroup("Talking Head");
    mst = app.project.activeItem;
    if (mst) {
      if (mst.selectedLayers.length > 0) {
        ...
        alert(
          "No Layer Selected\rPlease select a layer to use as the " + jaw_n,
        );
      }
    } else {
      alert("Please open a composition");
  ...
  f.rigFace = e.add("button", undefined, "Face").onClick = function () {
    app.beginUndoGroup("Talking Head");
    mst = app.project.activeItem;
    if (mst) {
      if (mst.selectedLayers.length > 0) {
        var l = mst.selectedLayers[0];
        l.name = face_n;
        checkForControls(mst);
        var k = new Array();
        k = k.concat(getFeatures(pupil_n, l.index));
        k = k.concat(getFeatures(eye_n, l.index));
        k = k.concat(getFeatures(brow_n, l.index));
        k = k.concat(getFeatures(mouth_n, l.index));
        k = k.concat(getFeatures(jaw_n, l.index));
        attachToFace(k, l);
      } else {
        alert(
          "No Layer Selected\rPlease select a layer to use as the " + face_n,
        );
      }
    } else {
      alert("Please open a composition");
  ...
  c.orientation = "row";
  c.alignment = "right";
  c.add("button", undefined, "?").onClick = function () {
    thx2020.helpUI();
  };
  f.layout.layout(true);
  return f;
}
function rc_TalkingHeadLicFunc(c) {
  if (thx2020.c()) {
    rc_TalkingHeadMainFunc(c);
  }
}
function a(vars) {
  function licUI() {
  ...
  var maxUIButtons = 3;
  var licV = 2;
  var wx = __BLOB__BLOB_000688__;
  var mx = __BLOB__BLOB_000689__;
...
var exp_header = "//      Talking Head Expression \r";
var ctrl_name = "Talking Head Controls";
var default_sel = "- Empty -";
var no_sel = "Select A Layer";
var globalVersion = "2.0.1";
var globalName = "Talking Head";
var script_nfo = globalName + " v" + globalVersion;
var aud_n = "Audio Amplitude";
var mot_n = "Mouth Open Threshold";
var lms_n = "Link Mouth Shape to Volume";
var moc_n = "Mouth Open Count";
var msf_n = "Mouth Scale Factor";
var mo_n = "Mouth Shape Open";
var mc_n = "Mouth Shape Closed";
var jsf_n = "Jaw Scale Factor";
var ld_n = "Look Direction";
var ls_n = "Look Skew";
var bl_int = "Blink Interval";
var bl_sp = "Blink Speed";
var sq_n = "Squint";
var sqos_n = "Squint Offset";
var ebt_n = "Eyebrows Tilt";
var ebr_n = "Eyebrows Raise";
var ebos_n = "Eyebrows Offset";
var ebp_n = "Eyebrows Pinch";
var pupil_n = "Pupil";
var brow_n = "Eyebrow";
var eye_n = "Eye";
var mouth_n = "Mouth";
...
var th_settings = {
  ...
  helpButtons: [],
  helpText:
    'Open a composition with facial feature layers.  Select facial feature layers one at a time, assign to rig by pressing corresponding facial feature button in the Talking Head Script UI Panel.\r\rFor lip sync add sound file to composition and select "Convert Audio to Keyframes"',
  offerBeta: false,
  offerTrial: true,
  privateNumber: 7335763515975332,
  productSKU: "RCTH-SUL",
  scriptAuthor: "Ryan Cashman",
  scriptName: globalName,
  scriptURL: "http://aescripts.com/talking-head/",
  scriptVersion: globalVersion,
};
var thx2020 = new a(th_settings);
rc_TalkingHeadLicFunc(this);
