...
var markerMaestro = function (thisObj) {
  var af_settings = {
    ...
    helpButtons: [],
    helpText:
      "MarkerMaestro Documentation\n\nEDIT TAB\nThe Marker List shows all the markers in the selected Comp or Layer. To edit a marker, double-click the item in the Marker List. To change the marker list, select another layer or deselect all to show the Comp\'s layer, then click on the MarkerMaestro window. MarkerMaestro will not update until you click back on the window.\n\nTo shift markers forward or backward on the timeline, select the markers you want to move, set the amount of time you want to shift the markers in the shift window using timecode, then click the Shift Marker(s) button. Note: timecode may be entered using shorthand. For example, if you enter 1.0 MarkerMaestro will convert that to 00:00:01:00, and your markers will be shifted forward by one second.\n\nClicking the CTI button will move the play head to the selected marker\'s time. If you have more than one marker selected, the play head will move to the first selected marker.\n\nClicking the + button will open a marker window and will let you add a marker at the current time. \n\nClicking the - button will delete the selected markers.\n\nDUPLICATE TAB\nThe Project List shows all of the compositions and their nested layers, in your project. If there is a pre-composed layer in a Comp, it will display its nested layers as well. Select the Comp or Comp layer you want to copy the selected markers to, click copy, and MarkerMaestro will duplicate the selected layers to the destination of your choice.\n",
    offerBeta: false,
    offerTrial: true,
    privateNumber: 6647851492880384,
    productSKU: "RWMM-SUL",
    scriptAuthor: "Robert Womack",
    scriptName: "MarkerMaestro",
    scriptURL: "http://aescripts.com/markermaestro/",
    scriptVersion: "1.0",
  };
  function a(vars) {
    function licUI() {
    ...
    var maxUIButtons = 3;
    var licV = 2;
    var mx = __BLOB__BLOB_000373__;
    var wx = __BLOB__BLOB_000374__;
  ...
  var markerMaestro_af_settings = new a(af_settings);
  var isTrial = markerMaestro_af_settings.t();
  if (markerMaestro_af_settings.c()) {
    var build_markerMaestro_UI = function (thisObj) {
      function listMarkers() {
        for (var i = 1; i <= myMarkers.numKeys; i += 1) {
          listbox1_array.push(myMarkers.keyValue(i).comment);
        }
      }
      var markerMaestroUI =
        thisObj instanceof Panel
          ? thisObj
          : new Window("window", "Marker Maestro", undefined, {
              maximizeButton: true,
              minimizeButton: true,
              resizeable: true,
            });
      markerMaestroUI.preferredSize.width = 200;
      markerMaestroUI.orientation = "column";
      markerMaestroUI.alignChildren = ["left", "top"];
      markerMaestroUI.spacing = 10;
      markerMaestroUI.margins = 16;
      var titleGroup = markerMaestroUI.add("group", undefined, {
        name: "titleGroup",
      });
      ...
      var image1_imgString = img_please("comp_icn", __BLOB__BLOB_000375__);
      var image1 = titleGroup.add("image", undefined, image1_imgString, {
        name: "image1",
      });
      image1.visible = myItem.indexOf("Layer") > -1 ? false : true;
      var statictext1 = titleGroup.add("statictext", undefined, undefined, {
        name: "statictext1",
      });
      statictext1.text = myItem;
      var tpanel1 = markerMaestroUI.add("tabbedpanel", undefined, undefined, {
        name: "tpanel1",
      });
      tpanel1.alignChildren = "fill";
      tpanel1.preferredSize.width = 285;
      tpanel1.margins = 0;
      ...
      tab1.spacing = 10;
      tab1.margins = 10;
      var listbox1_array = ["-- Markers --"];
      listMarkers();
      var listbox1 = tab1.add("listbox", undefined, undefined, {
        items: listbox1_array,
        multiselect: true,
        name: "listbox1",
      });
      listbox1.preferredSize.width = 250;
      listbox1.items[0].enabled = false;
      listbox1.onDoubleClick = function () {
        mm_Marker_Window_open("edit");
      };
      listbox1.helpTip = "Double click to edit";
      var repopulate_listbox = function (newtimes) {
        if (markerMaestro_af_settings.s() === false) {
          isTrial = true;
        }
        listbox1.removeAll();
        if (myLayer.length > 0) {
          myMarkers = myLayer[0].property("Marker");
        } else {
          myMarkers = myComp.markerProperty;
        }
        var init_val = listbox1.add("item", "-- Markers --", 0);
        init_val.enabled = false;
        var mySelection = [];
        for (var x = 1; x <= myMarkers.numKeys; x += 1) {
          listbox1.add("item", myMarkers.keyValue(x).comment, x);
          if (newtimes != undefined) {
            for (var y = 0; y < newtimes.length; y += 1) {
              if (myMarkers.keyTime(x).toFixed(5) == newtimes[y].toFixed(5)) {
                mySelection.push(x);
                break;
              }
            }
        ...
        listbox1.selection = mySelection;
      };
      markerMaestroUI.onActivate = function () {
        myLayer = myComp.selectedLayers;
        if (myLayer.length > 0) {
          myMarkers = myLayer[0].property("Marker");
          myItem = "Layer " + myLayer[0].index + ". " + myLayer[0].name;
        } else {
          myMarkers = myComp.markerProperty;
          myItem = myComp.name;
        }
      ...
      var ToolGroup = tab1.add("group", undefined, { name: "ToolGroup" });
      ToolGroup.orientation = "row";
      ToolGroup.alignChildren = ["left", "center"];
      ToolGroup.spacing = 2;
      ToolGroup.margins = 0;
      var mm_Move_button_imgString = img_please(
        "move_icn",
        __BLOB__BLOB_000376__,
      );
      var mm_Move_button = ToolGroup.add(
        "iconbutton",
        undefined,
        mm_Move_button_imgString,
        { name: "mm_Move_button", style: "toolbutton" },
      );
      mm_Move_button.enabled = false;
      mm_Move_button.helpTip = "Shift marker(s)";
      var mm_Move_field = ToolGroup.add(
        'edittext {justify: "right", properties: {name: "mm_Move_field"}}',
      );
      mm_Move_field.enabled = false;
      mm_Move_field.helpTip = "Shift marker(s) by this amount in timecode";
      mm_Move_field.text = format_timecode(correct_timecode("1.0"));
      mm_Move_field.preferredSize.width = 73;
      mm_Move_field.onDeactivate = function () {
        var rev = "";
        var txt = "";
        if (this.text.indexOf("-") == 0) {
          rev = "-";
          txt = this.text.substr(1, this.text.length);
        } else {
          txt = this.text;
        }
        this.text = rev + format_timecode(correct_timecode(txt));
      };
      mm_Move_button.onClick = mm_Move_field.onEnterKey = function () {
        app.beginUndoGroup("Move Markers");
        var newtimes = [];
        var myIndex = 0;
        var newTime = 0;
        if (mm_Move_field.text.indexOf("-") > -1) {
          for (var i = 0; i < listbox1.selection.length; i += 1) {
            myIndex = listbox1.selection[i].index;
            newTime =
              myMarkers.keyTime(myIndex) +
              process_move_field_value(mm_Move_field.text);
            newtimes.push(newTime);
            myNewMarker = new MarkerValue(myMarkers.keyValue(myIndex).comment);
            myMarkers.removeKey(myIndex);
            myMarkers.setValueAtTime(newTime, myNewMarker);
          }
        } else {
          for (i = listbox1.selection.length - 1; i > -1; i--) {
            myIndex = listbox1.selection[i].index;
            newTime =
              myMarkers.keyTime(myIndex) +
              process_move_field_value(mm_Move_field.text);
        ...
        repopulate_listbox(newtimes);
        app.endUndoGroup();
      };
      var divider1 = ToolGroup.add("panel", undefined, undefined, {
        name: "divider1",
      });
      divider1.alignment = "fill";
      var mm_CTI_button_imgString = img_please(
        "cti_icn",
        __BLOB__BLOB_000377__,
      );
      var mm_CTI_button = ToolGroup.add(
        "iconbutton",
        undefined,
        File.decode(mm_CTI_button_imgString),
        { name: "mm_CTI_button", style: "toolbutton" },
      );
      mm_CTI_button.enabled = false;
      mm_CTI_button.helpTip = "Move playhead to marker";
      mm_CTI_button.onClick = function () {
        var myIndex = listbox1.selection[0].index;
        myComp.time = myMarkers.keyTime(myIndex);
      };
      var mm_Add_button_imgString = img_please(
        "add_icn",
        __BLOB__BLOB_000378__,
      );
      var mm_Add_button = ToolGroup.add(
        "iconbutton",
        undefined,
        File.decode(mm_Add_button_imgString),
        { name: "mm_Add_button", style: "toolbutton" },
      );
      mm_Add_button.helpTip = "Add marker";
      mm_Add_button.onClick = function () {
        mm_Marker_Window_open();
      };
      var mm_Delete_button_imgString = img_please(
        "delete_icn",
        __BLOB__BLOB_000379__,
      );
      var mm_Delete_button = ToolGroup.add(
        "iconbutton",
        undefined,
        File.decode(mm_Delete_button_imgString),
        { name: "mm_Delete_button", style: "toolbutton" },
      );
      mm_Delete_button.enabled = false;
      mm_Delete_button.helpTip = "Delete selected markers";
      mm_Delete_button.onClick = function () {
        app.beginUndoGroup("Delete Markers");
        var arr = [];
        for (var x = 0; x < listbox1.selection.length; x += 1) {
          arr.push(listbox1.selection[x].index);
        }
        ...
        var init_val = listbox1.add("item", "-- Markers --", 0);
        init_val.enabled = false;
        for (var k = 1; k <= myMarkers.numKeys; k += 1) {
          listbox1.add("item", myMarkers.keyValue(k).comment);
        }
        app.endUndoGroup();
      };
      var mm_Marker_Window_open = function (edit) {
        var mm_MarkerEditCommentText = "";
        if (edit == "edit") {
          myIndex = listbox1.selection[0].index;
          var myProps = myMarkers.keyValue(myIndex);
          mm_MarkerEditTimeDisplay = timeToCurrentFormat(
            myMarkers.keyTime(myIndex),
            $framerate,
          );
          mm_MarkerEditDurationLabelDisplay = timeToCurrentFormat(
            myProps.duration,
            $framerate,
          );
          mm_MarkerEditCommentText = myProps.comment;
          mm_MarkerEditLabelSelection = myProps.label;
          mm_MarkerEditProtectedRegionValue = myProps.protectedRegion;
        } else {
          mm_MarkerEditTimeDisplay = timeToCurrentFormat(
            myComp.time,
            $framerate,
          ...
          mm_MarkerEditProtectedRegionValue = false;
        }
        var mm_MarkerEditWindow = new Window("dialog");
        mm_MarkerEditWindow.text = "Composition Marker";
        mm_MarkerEditWindow.orientation = "column";
        mm_MarkerEditWindow.alignChildren = ["left", "top"];
        mm_MarkerEditWindow.spacing = 10;
        mm_MarkerEditWindow.margins = 16;
        var mm_MarkerEditWindow_group1 = mm_MarkerEditWindow.add(
          "group",
          undefined,
          { name: "mm_MarkerEditWindow_group1" },
        );
        mm_MarkerEditWindow_group1.orientation = "row";
        mm_MarkerEditWindow_group1.alignChildren = ["left", "center"];
        mm_MarkerEditWindow_group1.spacing = 10;
        mm_MarkerEditWindow_group1.margins = 0;
        var mm_MarkerEditTimeLabel = mm_MarkerEditWindow_group1.add(
          "statictext",
          undefined,
          undefined,
          { name: "mm_MarkerEditTimeLabel" },
        );
        mm_MarkerEditTimeLabel.text = "Time:";
        mm_MarkerEditTimeLabel.preferredSize.width = 65;
        mm_MarkerEditTimeLabel.justify = "right";
        var mm_MarkerEditTime = mm_MarkerEditWindow_group1.add(
          'edittext {properties: {name: "mm_MarkerEditTime"}}',
        );
        mm_MarkerEditTime.text = mm_MarkerEditTimeDisplay;
        mm_MarkerEditTime.onDeactivate = function () {
          this.text = format_timecode(correct_timecode(this.text));
        };
        var mm_MarkerEditTimeDisclaimer = mm_MarkerEditWindow_group1.add(
          "statictext",
          undefined,
          undefined,
          { name: "mm_MarkerEditTimeDisclaimer" },
        );
        var dropf = "";
        var timeBase = Math.round($framerate);
        if (timeBase == 30 || timeBase == 60) {
          dropf = myComp.dropFrame == true ? "drop" : "non-drop";
        } else {
          if (timeBase == 24) {
            dropf = "non-drop";
          }
        }
        mm_MarkerEditTimeDisclaimer.text =
          "is " + mm_MarkerEditTimeDisplay + " Base " + timeBase + " " + dropf;
        var mm_MarkerEditWindow_group2 = mm_MarkerEditWindow.add(
          "group",
          undefined,
          { name: "mm_MarkerEditWindow_group2" },
        );
        mm_MarkerEditWindow_group2.orientation = "row";
        mm_MarkerEditWindow_group2.alignChildren = ["left", "center"];
        mm_MarkerEditWindow_group2.spacing = 10;
        mm_MarkerEditWindow_group2.margins = 0;
        var mm_MarkerEditDurationLabel = mm_MarkerEditWindow_group2.add(
          "statictext",
          undefined,
          undefined,
          { name: "mm_MarkerEditDuration" },
        );
        mm_MarkerEditDurationLabel.text = "Duration:";
        mm_MarkerEditDurationLabel.preferredSize.width = 65;
        mm_MarkerEditDurationLabel.justify = "right";
        var mm_MarkerEditDuration = mm_MarkerEditWindow_group2.add(
          'edittext {properties: {name: "mm_MarkerEditDurationLabel"}}',
        );
        mm_MarkerEditDuration.text = mm_MarkerEditDurationLabelDisplay;
        mm_MarkerEditDuration.onDeactivate = function () {
          this.text = format_timecode(correct_timecode(this.text));
        };
        var mm_MarkerEditDurationDisclaimer = mm_MarkerEditWindow_group2.add(
          "statictext",
          undefined,
          undefined,
          { name: "mm_MarkerEditDurationDisclaimer" },
        );
        mm_MarkerEditDurationDisclaimer.text =
          "is " +
          mm_MarkerEditDurationLabelDisplay +
          " Base " +
          timeBase +
          " " +
          dropf;
        var mm_MarkerEditWindow_group3 = mm_MarkerEditWindow.add(
          "group",
          undefined,
          { name: "mm_MarkerEditWindow_group3" },
        );
        mm_MarkerEditWindow_group3.orientation = "row";
        mm_MarkerEditWindow_group3.alignChildren = ["left", "top"];
        mm_MarkerEditWindow_group3.spacing = 10;
        mm_MarkerEditWindow_group3.margins = 0;
        var mm_MarkerEditCommentLabel = mm_MarkerEditWindow_group3.add(
          "statictext",
          undefined,
          undefined,
          { name: "mm_MarkerEditCommentLabel" },
        );
        mm_MarkerEditCommentLabel.text = "Comment:";
        mm_MarkerEditCommentLabel.preferredSize.width = 65;
        mm_MarkerEditCommentLabel.justify = "right";
        var mm_MarkerEditComment = mm_MarkerEditWindow_group3.add(
          'edittext {properties: {name: "mm_MarkerEditComment", multiline: true}}',
        );
        mm_MarkerEditComment.preferredSize.width = 350;
        mm_MarkerEditComment.preferredSize.height = 100;
        mm_MarkerEditComment.text = mm_MarkerEditCommentText;
        var mm_MarkerEditWindow_group4 = mm_MarkerEditWindow.add(
          "group",
          undefined,
          { name: "mm_MarkerEditWindow_group4" },
        );
        mm_MarkerEditWindow_group4.orientation = "row";
        mm_MarkerEditWindow_group4.alignChildren = ["left", "top"];
        mm_MarkerEditWindow_group4.spacing = 10;
        mm_MarkerEditWindow_group4.margins = 0;
        var mm_MarkerEditLabelLabel = mm_MarkerEditWindow_group4.add(
          "statictext",
          undefined,
          undefined,
          { name: "mm_MarkerEditLabelLabel" },
        );
        mm_MarkerEditLabelLabel.text = "Label:";
        mm_MarkerEditLabelLabel.preferredSize.width = 65;
        mm_MarkerEditLabelLabel.justify = "right";
        var mm_MarkerEditLabel_array = [
          "None",
          "Red",
          "Yellow",
          "Aqua",
        ...
        var mm_MarkerEditLabel = mm_MarkerEditWindow_group4.add(
          "dropdownlist",
          undefined,
          undefined,
          { items: mm_MarkerEditLabel_array, name: "mm_MarkerEditLabel" },
        );
        mm_MarkerEditLabel.selection = mm_MarkerEditLabelSelection;
        var mm_MarkerEditResponsivePanel = mm_MarkerEditWindow.add(
          "panel",
          undefined,
          undefined,
          { name: "mm_MarkerEditResponsivePanel" },
        );
        mm_MarkerEditResponsivePanel.text = "Responsive Design \u2013 Time";
        mm_MarkerEditResponsivePanel.preferredSize.width = 425;
        mm_MarkerEditResponsivePanel.orientation = "row";
        mm_MarkerEditResponsivePanel.alignChildren = ["left", "top"];
        mm_MarkerEditResponsivePanel.spacing = 10;
        mm_MarkerEditResponsivePanel.margins = 10;
        var mm_MarkerEditProtectedRegion = mm_MarkerEditResponsivePanel.add(
          "checkbox",
          undefined,
          undefined,
          { name: "mm_MarkerEditProtectedRegion" },
        );
        mm_MarkerEditProtectedRegion.text = "Protected Region";
        mm_MarkerEditProtectedRegion.value = mm_MarkerEditProtectedRegionValue;
        var mm_MarkerEditWindow_group5 = mm_MarkerEditWindow.add(
          "group",
          undefined,
          { name: "mm_MarkerEditWindow_group5" },
        );
        mm_MarkerEditWindow_group5.preferredSize.width = 425;
        mm_MarkerEditWindow_group5.orientation = "row";
        mm_MarkerEditWindow_group5.alignChildren = ["right", "center"];
        mm_MarkerEditWindow_group5.spacing = 10;
        mm_MarkerEditWindow_group5.margins = 0;
        var mm_MarkerEditWindowCancelButton = mm_MarkerEditWindow_group5.add(
          "button",
          undefined,
          undefined,
          { name: "button1" },
        );
        mm_MarkerEditWindowCancelButton.text = "Cancel";
        var mm_MarkerEditWindowOkButton = mm_MarkerEditWindow_group5.add(
          "button",
          undefined,
          undefined,
          { name: "button2" },
        );
        mm_MarkerEditWindowOkButton.text = "OK";
        var mm_getMarkerInfo = function () {
          var obj = new MarkerValue(mm_MarkerEditComment.text);
          obj.label = mm_MarkerEditLabel.selection.index;
          obj.protectedRegion = mm_MarkerEditProtectedRegion.value;
          var _duration = currentFormatToTime(
            mm_MarkerEditDuration.text,
            $framerate,
          );
          obj.duration =
            _duration == 0 && obj.protectedRegion == true ? 1 : _duration;
          return obj;
        };
        if (edit) {
          mm_MarkerEditWindowOkButton.onClick = function () {
            app.beginUndoGroup("Edit Marker");
            myMarkers.removeKey(myIndex);
            var myEditedMarker = mm_getMarkerInfo();
            var t =
              mm_MarkerEditTime.text.indexOf("-") == 0
                ? currentFormatToTime(
                    mm_MarkerEditTime.text.substr(1),
                    $framerate,
                  ) * -1
                : currentFormatToTime(mm_MarkerEditTime.text, $framerate);
            myMarkers.setValueAtTime(t, myEditedMarker);
            mm_MarkerEditWindow.close();
            repopulate_listbox();
            app.endUndoGroup();
          };
        } else {
          mm_MarkerEditWindowOkButton.onClick = function () {
            ...
            var myAddedMarker = mm_getMarkerInfo();
            var t =
              mm_MarkerEditTime.text.indexOf("-") == 0
                ? currentFormatToTime(
                    mm_MarkerEditTime.text.substr(1),
                ...
                : currentFormatToTime(mm_MarkerEditTime.text, $framerate);
            myMarkers.setValueAtTime(t, myAddedMarker);
            mm_MarkerEditWindow.close();
            repopulate_listbox();
            app.endUndoGroup();
          };
        }
        mm_MarkerEditWindow.show();
      };
      listbox1.onChange = function () {
        ...
        if (listbox1.selection.length > 0) {
          for (var i = 0; i <= button_array.length; i += 1) {
            button_array[i].enabled = true;
          }
        }
        mm_Move_field.active = true;
      };
      ...
      var statictext2 = tab2.add("statictext", undefined, undefined, {
        name: "statictext2",
      });
      statictext2.text = "Copy markers to:";
      var CompList = tab2.add("treeview", [0, 0, 250, 175], undefined, {
        name: "CompList",
      });
      var project_iteration = function (comp, node) {
        for (var j = 1; j <= comp.numLayers; j += 1) {
          if (comp.layers[j].source instanceof CompItem) {
            var treeN = node.add("node", comp.layers[j].name);
            treeN.loc = comp.layers[j].source;
            project_iteration(comp.layers[j].source, treeN);
          } else {
            var leaf = node.add("item", comp.layers[j].name);
            leaf.loc = comp.layers[j];
          }
        }
      };
      var populate_tree = function () {
        for (var i = 1; i <= app.project.numItems; i += 1) {
          if (app.project.item(i) instanceof CompItem) {
            if (app.project.item(i).usedIn.length == 0) {
              var treeN = CompList.add("node", app.project.item(i).name);
              treeN.loc = app.project.item(i);
              project_iteration(app.project.item(i), treeN);
            }
          }
        }
      };
      tab2.onSelect = populate_tree();
      var button1 = tab2.add("button", undefined, undefined, {
        name: "button1",
      });
      ...
      button1.onClick = function () {
        if (isTrial) {
          ...
          app.beginUndoGroup("Duplicate Markers");
          var target = CompList.selection.loc;
          var myMarkersTarget =
            target instanceof CompItem
              ? target.markerProperty
              : target.property("Marker");
          for (var i = listbox1.selection.length - 1; i > -1; i--) {
            var myIndex = listbox1.selection[i].index;
            var myNewMarker = new MarkerValue(
              myMarkers.keyValue(myIndex).comment,
            );
            myMarkersTarget.setValueAtTime(
              myMarkers.keyTime(myIndex),
              myNewMarker,
            );
          }
      ...
      var markerMaestro_af_help_button = markerMaestroUI.add(
        "button",
        undefined,
        undefined,
        { name: "markerMaestro_af_help_button" },
      );
      markerMaestro_af_help_button.text = "?";
      markerMaestro_af_help_button.preferredSize.width = 36;
      markerMaestro_af_help_button.alignment = ["right", "top"];
      markerMaestro_af_help_button.onClick = function () {
        markerMaestro_af_settings.helpUI();
      };
      ...
      return markerMaestroUI;
    };
    var re = function ($num) {
      var num_array = $num.toString().split("");
      var newArray = [];
      for (var i = 0; i < num_array.length; i += 2) {
        var d1 = num_array[i];
        var j = i;
        j++;
        if (num_array[j] != undefined) {
          d1 += num_array[j];
        }
        newArray.push(d1);
      }
    ...
    var format_timecode = function ($num) {
      var localRe = /(\d\d)/g;
      var $tc = [];
      var $timecode = $num.toString().split("").reverse().join("");
      $tc = re($timecode);
      ...
      for (var j = 0; j < 4; j += 1) {
        if ($tc[j] == undefined) {
          ...
          $tc[j] = $tc[j].split("").reverse().join("");
          $tc[j] = $tc[j] < 10 && $tc[j].length < 2 ? "0" + $tc[j] : $tc[j];
          $tc[j] = $tc[j].split("").reverse().join("");
        }
      }
      $timecode = $tc.join("").replace(localRe, "$1" + tc_delimiter);
      $timecode = $timecode.substring(0, $timecode.length - 1);
      $tc = $timecode.split("");
      return $tc.reverse().join("");
    };
    var correct_timecode = function ($num) {
      var strip_re = /(\d)0$/;
      var $time_divisors = new Array($framerate, 60, 60, 60);
      var $tcArray = [];
      var $tc = "";
      var my_re_1 = /^(\d)$/;
      $tcArray = $num.split(/\D/g);
      for (var h = 0; h < $tcArray.length; h += 1) {
        var t = $tcArray[h] == "" ? "00" : $tcArray[h].replace(my_re_1, "0$1");
        $tc += t;
      }
      var $ct = $tc.toString().split("").reverse().join("");
      $tcArray = re($ct);
      for (var i = 0; i < $tcArray.length; i += 1) {
        $tcArray[i] = $tcArray[i].toString().replace(strip_re, "$1");
        $tcArray[i] = $tcArray[i].split("").reverse().join("");
        if ($tcArray[i] >= $time_divisors[i]) {
          var j = i;
          j++;
          if ($tcArray[j] == undefined) {
            $tcArray[j] = parseInt(
              Math.floor($tcArray[i] / $time_divisors[i]),
              10,
            );
            $tcArray[j] = $tcArray[j].toString().split("").reverse().join("");
          } else {
            $tcArray[j] = $tcArray[j].replace(strip_re, "$1");
            $tcArray[j] =
              parseInt($tcArray[j].split("").reverse().join(""), 10) +
              parseInt(Math.floor($tcArray[i] / $time_divisors[i]), 10);
            $tcArray[j] = $tcArray[j].toString().split("").reverse().join("");
          }
          $tcArray[i] = parseInt($tcArray[i] % $time_divisors[i], 10);
          $tcArray[i] = $tcArray[i].toString();
        }
        if ($tcArray[i] < 10 && $tcArray[parseInt(i, 10) + 1] != undefined) {
          $tcArray[i] = "0" + $tcArray[i];
        }
        $tcArray[i] = $tcArray[i].split("").reverse().join("");
      }
      $ct = $tcArray.join("").split("").reverse().join("");
      return $ct;
    };
    var process_move_field_value = function ($num) {
      var rev = false;
      var txt = $num;
      if ($num.indexOf("-") == 0) {
        rev = true;
        txt = $num.substr(1, $num.length);
      }
      return rev
        ? currentFormatToTime(txt, $framerate) * -1
        : currentFormatToTime(txt, $framerate);
    };
    var img_please = function (img_name, binary_string) {
      myDataFolder = Folder(Folder.userData.absoluteURI + "/RW_Files");
      if (myDataFolder.exists == true) {
        myImg = File(myDataFolder.absoluteURI + "/" + img_name + ".png");
        if (myImg.exists == true) {
          return myImg;
        } else {
          myImg = img_writer(img_name, binary_string);
          return myImg;
        }
      } else {
        myDataFolder = new Folder(Folder.userData.absoluteURI + "/RW_Files");
        myDataFolder.create();
        myImg = img_writer(img_name, binary_string);
        return myImg;
      }
    };
    var img_writer = function (img_name, binary_string) {
      var myDataFolder = Folder(Folder.userData.absoluteURI + "/RW_Files");
      var myFile = new File(myDataFolder.absoluteURI + "/" + img_name + ".png");
      myFile.encoding = "BINARY";
      myFile.open("w");
      myFile.write(File.decode(binary_string));
      myFile.close();
      return myFile;
    };
    ...
    if (myComp == null) {
      RobsTAUalert(
        "Please select a composition before running " + TAUname + ".",
      );
    } else {
      var $framerate = myComp.frameRate;
      var tc_delimiter =
        ($framerate == 29.97 || $framerate == 59.94) && myComp.dropFrame == true
          ? ";"
          : ":";
      ...
      var mm = build_markerMaestro_UI(thisObj);
      if (mm instanceof Window) {
        mm.center();
        mm.show();
      } else {
        mm.layout.layout(true);
        mm.layout.resize();
      }
    }
  }
};
var TAUname = "Marker Maestro";
var RobsTAUalert = function (alert_txt) {
  var RobsTAUalertDialog = new Window("dialog");
  RobsTAUalertDialog.text = TAUname;
  RobsTAUalertDialog.orientation = "column";
  RobsTAUalertDialog.alignChildren = ["center", "top"];
  RobsTAUalertDialog.spacing = 10;
  RobsTAUalertDialog.margins = 16;
  var statictext1 = RobsTAUalertDialog.add("statictext", undefined, undefined, {
    name: "statictext1",
  });
  statictext1.text =
    alert_txt == "Trial"
      ? "This function is not available in the trial version of MarkerMaestro."
      : alert_txt;
  var button1 = RobsTAUalertDialog.add("button", undefined, undefined, {
    name: "button1",
  });
  button1.text = "OK";
  button1.justify = "right";
  button1.alignment = ["right", "top"];
  RobsTAUalertDialog.show();
};
if (
  ...
  RobsTAUalert(
    'Please Check the "Allow Scripts to Write Files and Access Network" checkbox in Preferences -> General',
  );
  app.executeCommand(2359);
}
markerMaestro(this);
