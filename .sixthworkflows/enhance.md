...
1. **Analyse Contextuelle & Détection d'Intention**
   ...
   - **Détection de Skill** : Analyser la nature de la tâche :
     - Si **Python Automation** (PyShiftAE, threading, CEP bridge) : Charger immédiatement `.sixthskills/pyshiftae/SKILL.md`.
     - Si **ExtendScript/JSX Development** (ScriptUI, shape layers, panels) : Charger `.sixthskills/ae-scripting-expert/SKILL.md`.
     - Si **Debugging** (bug, crash, erreur, performance) : Charger immédiatement `.sixthskills/debugging-strategies/SKILL.md` et identifier le runtime (Python vs ExtendScript) pour adapter les techniques.
     ...
     - Si **AE C++ SDK / Plugins** (AETK, suites AEGP, TaskScheduler natif) : Charger `.sixthskills/ae-cpp-sdk-architecture/SKILL.md`.
     - Si **C++ Templates / Métaprogrammation générique** (concepts, SFINAE, librairies génériques) : Charger `.sixthskills/cpp-templates-metaprogramming/SKILL.md`.
...
2. **Recherche Active de Documentation**
   - Identifier les règles spécifiques au projet via `read_file` sur `.sixthrules/01-codingstandards.md`.
   - Utiliser `grep_search` dans `docs/` pour trouver la documentation pertinente (ex: `docs/02-guides/`, `docs/04-reference/`).
...
3. **Synthèse et Rédaction Structurée (Prompt Engineering)**
   ...
   - **Mode Standard** :
     ...
     - **Contexte Projet** : Injecter explicitement les règles de `01-codingstandards.md` (PyShiftAE patterns, ES3 rules).
...
4. **Validation et Exécution**
   ...
   - Une fois validé :
     ...
     - Vérifier la conformité avec les standards de `01-codingstandards.md`.