...
**Actions Required:**
1. **Start with MCP Pull:** Call `fast_read_file(path="/home/kidpixel/After_Effects_Scripts_Plugins_Bundle/memory-bank/activeContext.md")`.
...
7. **Locking Instruction:** ALWAYS use absolute paths for memory-bank files in `/home/kidpixel/After_Effects_Scripts_Plugins_Bundle/memory-bank/`. Use EXCLUSIVELY the fast-filesystem MCP tools (fast_*). Do NOT attempt to read memory-bank files via regular filesystem tools (read_file).

---

## 2. File Structure & Responsibilities
Access these via `fast_read_file` or `fast_list_directory` using absolute paths in `/home/kidpixel/After_Effects_Scripts_Plugins_Bundle/memory-bank/`. Pour l'édition, utilisez les outils natifs `replace_file_content` ou `edit_file` de filesystem-agent.
...
-   **Archiving Tool:** Use `fast_write_file(path="/home/kidpixel/After_Effects_Scripts_Plugins_Bundle/memory-bank/archives/...")` to archive old entries. Never delete data without archiving via this tool first.
...
-   **Process:**
    ...
    4.  **Sync:** Call the native `replace_file_content` tool or `edit_file` from filesystem-agent (with `{ edits: [{ oldText, newText }] }`) on relevant files (usually `progress.md` and `activeContext.md`).
