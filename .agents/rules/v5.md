...
- **Available Tools**: Les outils disponibles sont : `fast_read_file`, `fast_write_file`, `fast_get_directory_tree`, `fast_list_directory`, `fast_read_multiple_files`, `fast_search_files`. Pour modifier les fichiers, utilisez les outils natifs de Windsurf (replace_file_content / multi_replace_file_content) ou edit_file du serveur filesystem-agent.
- **Reading**: Utilisez l'outil `fast_read_file` avec le paramètre `path` (chemin absolu).
...
- **Updating**: Utilisez l'outil natif d'édition ou `edit_file` (filesystem-agent) avec `path` (chemin absolu) pour synchroniser l'état du projet en fin de tâche.
- **Writing/Archiving**: Utilisez l'outil `fast_write_file` avec `path` (chemin absolu) uniquement pour l'initialisation ou l'archivage (voir `memorybankprotocol.md` pour les restrictions).
- **Project Listing**: Utilisez l'outil `fast_get_directory_tree` pour diagnostiquer les connexions et vérifier la disponibilité des projets.
- **File Listing**: Utilisez l'outil `fast_list_directory` pour explorer la structure des fichiers dans un projet.
...
- **Constant Reminder**: Tous les outils fast-filesystem nécessitent un paramètre `path` avec chemin absolu vers les fichiers memory-bank.
...
### 2.6 Advanced Tools Integration
- **Outils autorisés** : `shrimp-task-manager`, `sequentialthinking_tools`, `replace_file_content`/`multi_replace_file_content`, `fast_read_multiple_files`, `json_query_query_json` (qui prend en argument `json_data`), `json_query_search_keys`.
...
- **Gestion JSON** : "Interdiction de lire intégralement (`read_file`) les fichiers JSON volumineux (manifest, i18n). Utilise `json_query_query_json` pour extraire uniquement les nœuds pertinents avant modification."
