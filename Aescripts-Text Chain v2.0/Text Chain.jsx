...
function textChain_Wrapping(thisObj, settingsObject) {
  function myScrypt_buildUI(thisObj) {
    ...
    function gimmeFolder(folderName) {
      ...
      myTargetFolder = app.project.items.addFolder(folderName);
      return myTargetFolder;
    }
    function gimmeLayerSelection() {
      var activeComp = app.project.activeItem;
      var mySelection = [];
      for (var l = 1; l <= activeComp.numLayers; l += 1) {
        var curLayer = activeComp.layer(l);
        if (curLayer.selected === true) {
          mySelection.push(curLayer);
        }
      }
      return mySelection;
    }
    function gimmelayerSelection(counter, userInterface, newList) {
      for (var i = 0; i < counter; i += 1) {
        if (
          userInterface.children[2].children[i].children[1].selection != null
        ) {
          var curIndex =
            userInterface.children[2].children[i].children[1].selection.index;
          var curItem = userInterface.children[2].children[i].children[0].text;
          if (newList[curIndex] === undefined) {
            newList[curIndex] = curItem;
          } else {
            alert("please select a diferent position for:\r>>>" + curItem);
            return;
          }
        } else {
          alert(
            "please select a position for:\r>>> " +
              userInterface.children[1].children[i].children[0].text,
          );
        }
      }
      return newList;
    }
    function leftRightExpression(localDriversName, globalDriversName) {
      var expression =
        'var driver = thisComp.layer("' +
        localDriversName +
        '");\r' +
        "var driverWidth = driver.sourceRectAtTime().width;\r" +
        "var driverPosition = driver.transform.position;\r" +
        'var localTracking = effect("Local tracking")("Slider");\r' +
        'var globalTracking = thisComp.layer("' +
        globalDriversName +
        '")("ADBE Effect Parade")("Pseudo/0.44330938377816453")("Pseudo/0.44330938377816453-0001");\r' +
        "var tracking = localTracking + globalTracking;\r" +
        "[driverPosition[0] + driverWidth + tracking, driverPosition[1]];";
      return expression;
    }
    function rightLeftExpression(localDriversName, globalDriversName) {
      var expression =
        'var driver = thisComp.layer("' +
        localDriversName +
        '");\r' +
    ...
    function maxWidthEx_whichLayers(list) {
      var myString = "";
      for (var li = 0; li < list.length; li += 1) {
        myString =
          ...
          ' = thisComp.layer("' +
          list[li].name +
          '").sourceRectAtTime().width;\r';
      }
      return myString;
    }
    function maxWidthEx_fullWidth(list) {
      var myString = "var fullWidth = text1";
      for (var li = 1; li < list.length; li += 1) {
        myString = myString.toString() + "+ text" + (li + 1).toString();
      }
      myString = myString.toString() + ";";
      return myString.toString();
    }
    function maxWidthEx_maxWidth() {
      return '\rvar maxWidth = thisLayer("ADBE Effect Parade")("Pseudo/0.44330938377816453")("Pseudo/0.44330938377816453-0004");\r\r';
    }
    function maxWidthExpression(list) {
      var expression =
        maxWidthEx_whichLayers(list).toString() +
        maxWidthEx_fullWidth(list).toString() +
        maxWidthEx_maxWidth();
      expression =
        expression.toString() +
        'if(thisLayer("ADBE Effect Parade")("Pseudo/0.44330938377816453")("Pseudo/0.44330938377816453-0003").value === 0) {\r' +
        " [100, 100];\r" +
        "} else {\r" +
        " if (fullWidth > maxWidth ){\r" +
        "  var difference = fullWidth - maxWidth;\r" +
        "  var amountOff = difference / (fullWidth/100);\r" +
        '   if(thisLayer("ADBE Effect Parade")("Pseudo/0.44330938377816453")("Pseudo/0.44330938377816453-0005").value === 0) {\r' +
        "     [100-amountOff ,100];\r" +
        "   } else {\r" +
        "     [100-amountOff, 100-amountOff];\r" +
        "   }\r" +
        " } else {\r" +
        "  [100,100];\r" +
        " }\r" +
    ...
    function maxWidthExpressionTRIAL(list) {
      var expression =
        maxWidthEx_whichLayers(list).toString() +
        maxWidthEx_fullWidth(list).toString();
      expression =
        expression.toString() +
        "\rvar maxWidth = 750;\r" +
        'if(thisLayer("ADBE Effect Parade")("Pseudo/0.3040041512689813")("Pseudo/0.3040041512689813-0002").value === 0) {\r' +
        " [100, 100];\r" +
        "} else {\r" +
        " if (fullWidth > maxWidth ){\r" +
        "  var difference = fullWidth - maxWidth;\r" +
    ...
    function setAlignmentRight(myTextLayer) {
      if (myTextLayer instanceof TextLayer) {
        ...
        alert("please select a valid Text Layer");
      }
    }
    function setAlignmentLeft(myTextLayer) {
      if (myTextLayer instanceof TextLayer) {
        var textProp = myTextLayer.property("Source Text");
        var textDocument = textProp.value;
    ...
    function leftRightExpressionTRIAL(localDriversName, globalDriversName) {
      var expression =
        "var driver = thisComp.layer(\'" +
        localDriversName +
        "\');\r" +
        ...
        "var globalTracking = thisComp.layer(\'" +
        globalDriversName +
        "\')(\'ADBE Effect Parade\')(\'Pseudo/0.3040041512689813\')(\'Pseudo/0.3040041512689813-0001\');\r" +
        "var tracking = localTracking + globalTracking;\r" +
        "[driverPosition[0] + driverWidth + tracking, driverPosition[1]];";
      return expression;
    }
    function rightLeftExpressionTRIAL(localDriversName, globalDriversName) {
      var expression =
        "var driver = thisComp.layer(\'" +
        localDriversName +
        "\');\r" +
    ...
    function checkPositDimensionsSeparated(mytextLayer) {
      if (
        myTextLayer.property("ADBE Transform Group").property("ADBE Position")
          .dimensionsSeparated === true
      ) {
        alert(myTextLayer.name + " have separated dimennsions");
        return;
      } else {
        alert(myTextLayer.name + " is fine!");
      }
    }
    function getOrCreate_PseudoFile(fileName, rawData) {
      ...
      function getUserDocumentsFolder() {
        return Folder.myDocuments.fsName.toString();
      }
      var filePath = getFilePath(fileName);
      var file = getFFXfile(filePath, rawData);
      return file;
    }
    function applyTextChainPseudo(myLayer_fxProperties) {
      function applyPseudoEffect(textChainPseudoEffect, myLayer_fxProperties) {
        var userDocumentsFolder = getUserDocumentsFolder();
        var animationPreset = createResourceFile(
          textChainPseudoEffect.presetName,
          textChainPseudoEffect.presetBinary,
          userDocumentsFolder,
        );
        var masterLayer = myLayer_fxProperties.parentProperty;
        ...
        textChainPseudoEffect.matchName = tempSolid
          .property("ADBE Effect Parade")
          .property(1).matchName;
        masterLayer.selected = true;
        myLayer_fxProperties.addProperty(textChainPseudoEffect.matchName);
      ...
      function getUserDocumentsFolder() {
        try {
          var userDocumentsFolder = getUserDocumentsFolder();
          var proprietarySubfolder = Folder(
            userDocumentsFolder.toString() +
              separator +
              "TextChain" +
              separator,
          );
          if (!proprietarySubfolder.exists) {
            var checkFolder = proprietarySubfolder.create();
            if (!checkFolder) {
              alert(
                'Error creating "' +
                  checkFolder.fsName +
              ...
              proprietarySubfolder = Folder.temp;
            }
          }
          return proprietarySubfolder.toString();
        } catch (err) {
          alert(
            "Permissions issue with user data folder\nPlease give AE full read and write permissions to the ~/Library/Application Support/Aescripts/Text Chain folder.",
          );
        }
      }
      ...
      var textChainPseudoEffect = {
        matchName: null,
        presetBinary: [__BLOB__BLOB_000690__],
        presetName: "Text Chain.ffx",
      };
      if (
        myLayer_fxProperties.canAddProperty(textChainPseudoEffect.matchName)
      ) {
        myLayer_fxProperties.addProperty(textChainPseudoEffect.matchName);
      } else {
        applyPseudoEffect(textChainPseudoEffect, myLayer_fxProperties);
      }
    }
    function applyTextChainPseudoTRIAL(myLayer_fxProperties) {
      function applyPseudoEffect(
        textChainPseudoEffectTRIAL,
        myLayer_fxProperties,
      ) {
        var userDocumentsFolder = getUserDocumentsFolder();
        var animationPreset = createResourceFile(
          textChainPseudoEffectTRIAL.presetName,
          textChainPseudoEffectTRIAL.presetBinary,
          userDocumentsFolder,
        );
        var masterLayer = myLayer_fxProperties.parentProperty;
        var curentComp = masterLayer.containingComp;
        ...
        var tempSolidFolder = tempSolidSource.parentFolder;
        tempSolid.applyPreset(File(animationPreset));
        textChainPseudoEffectTRIAL.matchName = tempSolid
          .property("ADBE Effect Parade")
          .property(1).matchName;
        masterLayer.selected = true;
        myLayer_fxProperties.addProperty(textChainPseudoEffectTRIAL.matchName);
        tempSolidSource.remove();
        if (tempSolidFolder.numItems === 0) {
          tempSolidFolder.remove();
        }
      ...
      function getUserDocumentsFolder() {
        try {
          var userDocumentsFolder = Folder.userData;
          var proprietarySubfolder = Folder(
            userDocumentsFolder.toString() + "/Aescripts/Text Chain/",
          );
          if (!proprietarySubfolder.exists) {
            var checkFolder = proprietarySubfolder.create();
            if (!checkFolder) {
      ...
      var textChainPseudoEffectTRIAL = {
        matchName: null,
        presetBinary: [__BLOB__BLOB_000691__],
        presetName: "Text Chain trial.ffx",
      };
      if (
        myLayer_fxProperties.canAddProperty(
          textChainPseudoEffectTRIAL.matchName,
        )
      ) {
        myLayer_fxProperties.addProperty(textChainPseudoEffectTRIAL.matchName);
      } else {
        applyPseudoEffect(textChainPseudoEffectTRIAL, myLayer_fxProperties);
      }
    }
    function createChain() {
      ...
      for (var l = 1; l <= activeComp.numLayers; l += 1) {
        var curLayer = activeComp.layer(l);
        if (curLayer.selected === true) {
          layerSelection.push(curLayer);
        }
      }
      var itemsNumber = layerSelection.length;
      if (itemsNumber <= 1) {
        alert("plase select at least 2 layers");
        return;
      }
      for (var x = 0; x < itemsNumber; x += 1) {
        if (
          layerSelection[x]
            .property("ADBE Transform Group")
            .property("ADBE Position").dimensionsSeparated === true
        ) {
          alert(
            "Looks like the layer \'" +
              layerSelection[x].name.toString() +
              "\' > Position\'s dimensions are separated\rUnfortunately Text Chain can\'t work in this case\rPlease de-select \'Separate Dimension\' from the property if you want to use Text Chain",
          );
          return;
        }
        if (
          layerSelection[x]
            ...
            .property("ADBE Text Document").numKeys > 0
        ) {
          alert(
            "Looks like the layer \'" +
              layerSelection[x].name.toString() +
              "\' > Source Text have keyframes apply to it\rUnfortunately Text Chain can\'t work in this case\rPlease delete any keyframe from the mentioned property if  you want to use Text Chain",
          );
          return;
        }
        var nullTargetName = layerSelection[x].name + "_CTRL";
        for (var y = 1; y < activeComp.numLayers; y += 1) {
          var curCTRL = activeComp.layer(y);
          if (curCTRL.name === nullTargetName) {
            curCTRL.remove();
          } else {
            if (curCTRL.name === layerSelection[x].toString()) {
              curCTRL
                .property("ADBE Transform Group")
      ...
      myNull = activeComp.layers.addNull();
      myNull.source.name = layerSelection[0].name + "_CTRL";
      var myPreset = getOrCreate_PseudoFile(
        "TextChain2.0.ffx",
        psudoBinaryCode_FFX,
      );
      alert(typeof myPreset);
      myNull.applyPreset(myPreset);
      var driver = activeComp.layer(layerSelection[0].name);
      var driverPosition = driver
        .property("ADBE Transform Group")
        .property("ADBE Position").value;
      myNull.moveBefore(driver);
      myNull.enabled = false;
      var driverIndex = driver.index;
      myNull
        .property("ADBE Transform Group")
        .property("ADBE Position")
        .setValue(driverPosition);
      var buildToExclude = "11.";
      if (app.buildName.indexOf(buildToExclude) !== 0) {
        myNull
          .property("ADBE Transform Group")
          .property("ADBE Scale").expression =
          maxWidthExpression(layerSelection);
      }
      for (var ls = 0; ls < itemsNumber; ls += 1) {
        var myLayer = activeComp.layer(layerSelection[ls].name);
        myLayer.parent = myNull;
        if (leftToRightButton.value === true) {
          var leftTextProp = myLayer
            .property("ADBE Text Properties")
            .property("ADBE Text Document");
          var leftTextDocument = leftTextProp.value;
          leftTextDocument.justification = ParagraphJustification.LEFT_JUSTIFY;
          leftTextProp.setValue(leftTextDocument);
        } else {
          if (rightToLeftButton.value === true) {
            var rightTextProp = myLayer
              .property("ADBE Text Properties")
              .property("ADBE Text Document");
            var rightTextDocument = rightTextProp.value;
            rightTextDocument.justification =
              ParagraphJustification.RIGHT_JUSTIFY;
            rightTextProp.setValue(rightTextDocument);
          }
        }
      }
      for (var i = 1; i < itemsNumber; i += 1) {
        var curitem = layerSelection[i].name;
        var layerFx = activeComp
          .layer(layerSelection[i].name)
          .property("ADBE Effect Parade");
        if (layerFx.numProperties > 0) {
          for (var fx = 1; fx <= layerFx.numProperties; fx += 1) {
            var curFX = layerFx.property(fx);
            if (curFX.matchName === "ADBE Slider Control") {
              if (curFX.name !== "Local tracking") {
                localSlider = layerFx.addProperty("ADBE Slider Control");
                localSlider.name = "Local tracking";
                localSlider.property("ADBE Slider Control-0001").setValue(0);
              } else {
                if (curFX.name === "Local tracking") {
                  curFX.property("ADBE Slider Control-0001").setValue(0);
                }
              }
            }
          }
        ...
        if (leftToRightButton.value === true) {
          activeComp
            .layer(curitem)
            .property("ADBE Transform Group")
            .property("ADBE Position").expression = leftRightExpression(
            layerSelection[i - 1].name,
            myNull.name,
          );
        } else {
          if (rightToLeftButton.value === true) {
            activeComp
              .layer(curitem)
              .property("ADBE Transform Group")
              .property("ADBE Position").expression = rightLeftExpression(
              layerSelection[i - 1].name,
              myNull.name,
            );
          }
      ...
      if (
        layerSelection[0]
          .property("ADBE Effect Parade")
          .property("ADBE Slider Control") !== null &&
        layerSelection[0]
          .property("ADBE Effect Parade")
    ...
    function createChain_Trial() {
      app.beginUndoGroup("link (trial) !");
      var activeComp = app.project.activeItem;
      if (!activeComp) {
        alert("please select a valid comp");
        return;
      ...
      if (itemsNumber <= 1) {
        ...
        if (itemsNumber > 2) {
          alert(
            "ERROR:\rTrial version creates only chains of 2 layers\rPlease select 2 layers to continue",
          );
          return;
        }
      }
      ...
      myNull.source.name = layerSelection[0].name + "_CTRL";
      var myPreset = getOrCreate_PseudoFile(
        "Text Chain trial.ffx",
        psudoBinaryTRIALCode_FFX,
      );
      myNull.applyPreset(myPreset);
      var driver = activeComp.layer(layerSelection[0].name);
      var driverPosition = driver
      ...
      myNull
        .property("ADBE Transform Group")
        .property("ADBE Scale").expression =
        maxWidthExpressionTRIAL(layerSelection);
      for (var ls = 0; ls < itemsNumber; ls += 1) {
        var myLayer = activeComp.layer(layerSelection[ls].name);
        myLayer.parent = myNull;
        if (leftToRightButton.value === true) {
      ...
      for (var i = 1; i < itemsNumber; i += 1) {
        ...
        if (leftToRightButton.value === true) {
          activeComp
            .layer(curitem)
            .property("ADBE Transform Group")
            .property("ADBE Position").expression = leftRightExpressionTRIAL(
            layerSelection[i - 1].name,
            myNull.name,
          );
        } else {
          if (rightToLeftButton.value === true) {
            activeComp
              .layer(curitem)
              .property("ADBE Transform Group")
              .property("ADBE Position").expression = rightLeftExpressionTRIAL(
              layerSelection[i - 1].name,
              myNull.name,
            );
          }
    ...
    var res =
      "group{orientation:\'column\', alignment: [\'top\', \'left\'], margins:5,buttonsGroup: Group{ orientation: \'row\', spacing: 5, alignment: [\'fill\', \'fill\'], margins: [0, 0, 0, -4]executeButton: Button{text: \'Link!\', alignment: [\'fill\', \'fill\']}helpButton: Button{text: \'?\', size : [28,28], alignment: [\'right\', \'center\']} }directionPanel: Panel{ orientation: \'row\', margins: [2, 6, 2, 2], spacing: 2, alignment: [\'left\', \'top\'], myText: StaticText {text: \'<<\', alignment: [\'left\', \'top\']},radialOne: RadioButton{text: \'direction\'},radialTwo: RadioButton{text: \'>>\', value: true},}}";
    ...
    var psudoBinaryCode_FFX = __BLOB__BLOB_000692__;
    var psudoBinaryTRIALCode_FFX = __BLOB__BLOB_000693__;
    var buttonsGroup = myPanel.grp.buttonsGroup;
    var linkButton = myPanel.grp.buttonsGroup.executeButton;
    var helpButton = myPanel.grp.buttonsGroup.helpButton;
    var radioPanel = myPanel.grp.directionPanel;
    var radioText = myPanel.grp.directionPanel.myText;
    var leftToRightButton = myPanel.grp.directionPanel.radialTwo;
    var rightToLeftButton = myPanel.grp.directionPanel.radialOne;
    if (OS.isWindows()) {
      separator = "\\";
    } else {
      if (OS.isMacOS()) {
    ...
    var helpMessage =
      "To link two or more layers in your timeline, you need to have a comp active, and in the timeline at least two text layers selected.\rBy default the direction is set to \u2018left to right\u2019, but you can change it if you want your text linked in the opposite direction.\rPlease note, whichever your text justification is set to, the script will change it according to the chosen \u2018direction\u2019\rThe text layers order in the titmeline is relevant: The layer with the smallest index will be the driver, the rest will follow in the index order\r\rA video tutorial is avilable online, please press the \'Yes\' button you want to open the link on Vimeo, or press \'No\' to continue using After Effects";
...
function dr_textChain(thisObj) {
  var af_settings = {
    helpText:
      "To link two or more layers in your timeline, you need to have a comp active, and in the timeline at least two text layers selected.\rBy default the direction is set to \u2018left to right\u2019, but you can change it if you want your text linked in the opposite direction.\rPlease note, whichever your text justification is set to, the script will change it according to the chosen \u2018direction\u2019\rThe text layers order in the titmeline is relevant: The layer with the smallest index will be the driver, the rest will follow in the index order\r\rPlease find the video tutorial here: vimeo.com/346004810",
    offerTrial: true,
    privateNumber: 5337147456676280,
    productSKU: "DRTC-SUL",
    scriptAuthor: "Daniele Ria",
    scriptName: "Text Chain",
    scriptURL: "https://aescripts.com/text-chain",
    scriptVersion: "2.0",
  };
  function a(vars) {
    function licUI() {
  ...
  if (goodLuck.c()) {
    textChain_Wrapping(thisObj, goodLuck);
  }
}
dr_textChain(this);
