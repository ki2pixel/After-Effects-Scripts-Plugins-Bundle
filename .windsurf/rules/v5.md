...
- **Available Tools**: Les 5 outils disponibles sont : `fast_read_file`, `edit_file`, `fast_write_file`, `fast_get_directory_tree`, `fast_list_directory`.
...
- **Updating**: Utilisez l'outil `edit_file` avec `path` (chemin absolu) pour synchroniser l'état du projet en fin de tâche.
...
- **Outils autorisés** : `task-master-ai`, `sequentialthinking_tools`, `edit_file`, `fast_read_multiple_files`, `json_query_jsonpath`, `json_query_search_keys`.
