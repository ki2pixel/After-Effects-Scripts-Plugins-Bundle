...
- **Available Tools**: Les 5 outils disponibles sont : `mcp0_fast_read_file`, `mcp0_fast_edit_block`, `mcp0_fast_write_file`, `mcp0_fast_get_directory_tree`, `mcp0_fast_list_directory`.
- **Reading**: Utilisez l'outil `mcp0_fast_read_file` avec le paramètre `path` (chemin absolu).
...
- **Updating**: Utilisez l'outil `mcp0_fast_edit_block` avec `path` (chemin absolu) pour synchroniser l'état du projet en fin de tâche.
- **Writing/Archiving**: Utilisez l'outil `mcp0_fast_write_file` avec `path` (chemin absolu) uniquement pour l'initialisation ou l'archivage (voir `memorybankprotocol.md` pour les restrictions).
- **Project Listing**: Utilisez l'outil `mcp0_fast_get_directory_tree` pour diagnostiquer les connexions et vérifier la disponibilité des projets.
- **File Listing**: Utilisez l'outil `mcp0_fast_list_directory` pour explorer la structure des fichiers dans un projet.
...
- **Outils autorisés** : `task-master-ai`, `sequentialthinking_tools`, `fast_edit_block`, `fast_read_multiple_files`, `json_query_jsonpath`, `json_query_search_keys`.
